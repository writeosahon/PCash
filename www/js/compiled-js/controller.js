...
utopiasoftware.saveup.controller = {
    ...
    appReady: () => {
        ...
        ons.ready(function () {
            ...
            // initialise terms-of-use-fixed-modal plugin
            $('#terms-of-use-fixed-modal').modal({
                ready: function(){ // callback for when fixed-modal is opened
                    // flag a state that indicates the modal is currently open
                    $('#terms-of-use-fixed-modal').data("fixedModalState", "open");
                },
                complete: function(){ // callback for when fixed-modal is closed
                    // flag a state that indicates the fixed-modal is currently closed
                    $('#terms-of-use-fixed-modal').data("fixedModalState", "closed");
                }
            });
        ...
        then(function(){
            // check identity of ssl certificates from critical servers used in the app
            return new Promise(function(resolve, reject){
                // create an array containing the promises for checking server
                var promisesArray = [];

                promisesArray.push(new Promise(function(resolve2, reject2){
                    window.plugins.sslCertificateChecker.check( // check ssl certificate for cedr
                        resolve2,
                        function(message){
                            if(message.indexOf("CONNECTION_FAILED") > -1){
                                resolve2();
                            }
                            else{
                                reject2();
                ...
                promisesArray.push(new Promise(function(resolve2, reject2){
                    window.plugins.sslCertificateChecker.check( // check ssl certificate for paystack
                        resolve2,
                        function(message){
                            if(message.indexOf("CONNECTION_FAILED") > -1){
                                resolve2();
                        ...
                        "36 7f e8 f1 48 97 87 81 9c 4f c7 d4 96 55 3e 1b 37 45 1e a7");
                }));

                // wait till all th ssl certificates have been verified
                Promise.all(promisesArray).then(function(){resolve();}).catch(function(){reject();});
            });
        }).
        then(function(){ // set up push notification for the app

            window.plugins.OneSignal
                .startInit("2058559d-c18d-4023-87da-b9b28dba1c96")
    ...
    /**
     * object is view-model for settings page
     */
        ...
        settingsMenuListClicked: function(label, event) {
            ...
            if(label == "change secure pin"){ // 'change secure pin' list item was clicked
                ...
                then(function(userInput){ // user has provided a secured PIN , now authenticate it
                    if(userInput === utopiasoftware.saveup.model.appUserDetails.securePin){ // authentication successful
                        $('#app-main-navigator').get(0).
                        pushPage("change-pin-page.html", {animation: "slide-md"}); // navigate
                    }
                    else{ // inform user that security check failed/user authentication failed
                        ons.notification.alert({title: "Security Check",
                            messageHTML: '<ons-icon icon="md-close-circle-o" size="30px" ' +
    ...
    changePinPageViewModel: {
        ...
        pageInit: function(event){
            ...
            function loadPageOnAppReady(){
                ...
                // initialise the change-pin form validation
                utopiasoftware.saveup.controller.changePinPageViewModel.formValidator = $('#change-pin-form').parsley();

                // attach listener for the change pin button on the page
                $('#change-pin-button').get(0).onclick = function(){
                    // run the validation method for the change pin form
                    utopiasoftware.saveup.controller.changePinPageViewModel.formValidator.whenValidate();
                };

                // listen for log in form field validation failure event
                utopiasoftware.saveup.controller.changePinPageViewModel.formValidator.on('field:error', function(fieldInstance) {
                    // get the element that triggered the field validation error and use it to display tooltip
                    // display tooltip
                    $(fieldInstance.$element).parent().find('label:eq(0)').addClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");
                    $(fieldInstance.$element).parent().find('label:eq(0)').attr("data-hint", fieldInstance.getErrorsMessages()[0]);
                ...
                // listen for form field validation success event
                utopiasoftware.saveup.controller.changePinPageViewModel.formValidator.on('field:success', function(fieldInstance) {
                    // remove tooltip from element
                    $(fieldInstance.$element).parent().find('label:eq(0)').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");
                    $(fieldInstance.$element).parent().find('label:eq(0)').removeAttr("data-hint");
                });
                ...
                // listen for form validation success
                utopiasoftware.saveup.controller.changePinPageViewModel.formValidator.on('form:success',
                    utopiasoftware.saveup.controller.changePinPageViewModel.changePinFormValidated);
        ...
        pageHide: (event) => {
            try {
                // remove any tooltip being displayed on all forms on the page
                $('#change-pin-page [data-hint]').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");
                $('#change-pin-page [data-hint]').removeAttr("data-hint");
                // reset the form validator object on the page
                utopiasoftware.saveup.controller.changePinPageViewModel.formValidator.reset();
            }
            catch(err){}
        },
        ...
        pageDestroy: (event) => {
            try{
                ...
                $('#change-pin-page [data-hint]').removeAttr("data-hint");
                // destroy the form validator objects on the page
                utopiasoftware.saveup.controller.changePinPageViewModel.formValidator.destroy();
            }
            catch(err){}
        },
        ...
        changePinFormValidated: function(){
            ...
            then(function(){
                ...
                utopiasoftware.saveup.model.appUserDetails.securePin =
                    $('#change-pin-form #change-pin-new-pin').val();
    ...
    appInfoPageViewModel: {
            ...
            function loadPageOnAppReady(){
                ...
                $('#app-main-navigator').get(0).topPage.onDeviceBackButton = function(){
                    ...
                    if($('#privacy-policy-fixed-modal').data("fixedModalState") === "open"){ // privacy-policy modal is open
                        ...
                        $('#privacy-policy-fixed-modal').modal("close"); // close the modal

                        return;
                    }

                    // check if the terms-of-use-fixed-modal is open
                    if($('#terms-of-use-fixed-modal').data("fixedModalState") === "open"){ // terms-of-use is open

                        $('#terms-of-use-fixed-modal .modal-content').scrollTop(0);
                        $('#terms-of-use-fixed-modal').modal("close"); // close the modal
        ...
        appInfoMenuListClicked: function(label) {
            ...
            if(label == "terms of use"){ // 'terms of use' list item was clicked
