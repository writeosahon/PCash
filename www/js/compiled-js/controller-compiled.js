'use strict';var _utopiasoftware$saveu;function _defineProperty(obj,key,value){if(key in obj){Object.defineProperty(obj,key,{value:value,enumerable:true,configurable:true,writable:true});}else{obj[key]=value;}return obj;}/**
...
utopiasoftware.saveup.controller=(_utopiasoftware$saveu={/**
...
$('#security-pin-lock-modal #security-pin-lock-pin').val("");return;// exit
...
$('#verify-account-bottom-sheet').data("saveupSheetState","closed");}});// add back button listener for the SECURE PIN LOCK MODAL
...
$('#transfer-cash-card-authorise-iframe').attr("src","");};// add back button listener for the transfer-cash-bank-authorise MODAL
$('#transfer-cash-bank-authorise-modal').get(0).onDeviceBackButton=function(){// hide the transfer-cash-bank-authorise-modal
$('#transfer-cash-bank-authorise-modal').get(0).hide();// change the src for transfer-cash-bank-authorise-iframe to reset
$('#transfer-cash-bank-authorise-iframe').attr("src","");};});/** ADD CUSTOM VALIDATORS FOR PARSLEY HERE **/Parsley.addAsyncValidator("financialcardcheck",utopiasoftware.saveup.controller.addCardPageViewModel.financialCardValidator,utopiasoftware.saveup.paystackObject.gateway+'decision/bin/{value}');Parsley.addAsyncValidator("accountnumbercheck",utopiasoftware.saveup.controller.addAccountPageViewModel.accountNumberValidator,utopiasoftware.saveup.moneyWaveObject.gateway+'v1/resolve/account');Parsley.addAsyncValidator("recipientaccountnumbercheck",utopiasoftware.saveup.controller.addRecipientPageViewModel.recipientAccountNumberValidator,utopiasoftware.saveup.moneyWaveObject.gateway+'v1/resolve/account');/** CUSTOM VALIDATORS FOR PARSLEY ENDS **/// add listener for when the Internet network connection is offline
...
position:"bottom",styling:{opacity:1,backgroundColor:'#000000',textColor:'#FFFFFF',textSize:14}});});},/**
     ...
     */createAppSecondaryMenus:function createAppSecondaryMenus(popOverQuerySelector,targetElem){// show the specified popover element
$(popOverQuerySelector).get(0).show(targetElem);},/**
     * method is triggered just before pin-security-check prompt dialog is shown
     */pinSecurityCheckPreShow:function pinSecurityCheckPreShow(){// find all input elements and ensure they are not mistakingly styled by materialize.css
$('#pin-security-check').find('input').addClass("utopiasoftware-no-style");},/**
     ...
     */appSecondaryMenuListClicked:function appSecondaryMenuListClicked(label){if(label=="sign out"){// user clicked the sign out list item
...
window.plugins.toast.hide();}});});return;}},/**
     ...
     */sideMenuViewModel:{/**
         ...
         */sideMenuListClicked:function sideMenuListClicked(label){if(label=="transfer cash"){// 'transfer cash' button was clicked
// close the side menu
...
// close the side menu
$('ons-splitter').get(0).left.close().then(function(){$('#app-main-navigator').get(0).bringPageTop("verify-account-page.html",{});// navigate to the verify account page
}).catch(function(){});return;}if(label=="my cards"){// 'my cards' button was clicked
// close the side menu
$('ons-splitter').get(0).left.close().then(function(){// ask user for secure PIN before proceeding. secure pin MUST match
return ons.notification.prompt({title:"Security Check",id:"pin-security-check",class:"utopiasoftware-no-style",messageHTML:'<div><ons-icon icon="ion-lock-combination" size="24px" '+'style="color: #b388ff; float: left; width: 26px;"></ons-icon> <span style="float: right; width: calc(100% - 26px);">'+'Please enter your PostCash Secure PIN to proceed</span></div>',cancelable:true,placeholder:"Secure PIN",inputType:"number",defaultValue:"",autofocus:true,submitOnEnter:true});}).then(function(userInput){// user has provided a secured PIN , now authenticate it
if(userInput===utopiasoftware.saveup.model.appUserDetails.securePin){// authentication successful
...
ons.notification.alert({title:"Security Check",messageHTML:'<ons-icon icon="md-close-circle-o" size="30px" '+'style="color: red;"></ons-icon> <span>'+'Security check failed. Invalid credentials'+'</span>',cancelable:true});}}).catch(function(){});return;}if(label=="my accounts"){// 'my accounts' button was clicked
// close the side menu
$('ons-splitter').get(0).left.close().then(function(){// ask user for secure PIN before proceeding. secure pin MUST match
return ons.notification.prompt({title:"Security Check",id:"pin-security-check",class:"utopiasoftware-no-style",messageHTML:'<div><ons-icon icon="ion-lock-combination" size="24px" '+'style="color: #b388ff; float: left; width: 26px;"></ons-icon> <span style="float: right; width: calc(100% - 26px);">'+'Please enter your PostCash Secure PIN to proceed</span></div>',cancelable:true,placeholder:"Secure PIN",inputType:"number",defaultValue:"",autofocus:true,submitOnEnter:true});}).then(function(userInput){// user has provided a secured PIN , now authenticate it
if(userInput===utopiasoftware.saveup.model.appUserDetails.securePin){// authentication successful
...
ons.notification.alert({title:"Security Check",messageHTML:'<ons-icon icon="md-close-circle-o" size="30px" '+'style="color: red;"></ons-icon> <span>'+'Security check failed. Invalid credentials'+'</span>',cancelable:true});}}).catch(function(){});return;}if(label=="saved recipients"){// 'saved recipients' button was clicked
// close the side menu
$('ons-splitter').get(0).left.close().then(function(){// ask user for secure PIN before proceeding. secure pin MUST match
return ons.notification.prompt({title:"Security Check",id:"pin-security-check",class:"utopiasoftware-no-style",messageHTML:'<div><ons-icon icon="ion-lock-combination" size="24px" '+'style="color: #b388ff; float: left; width: 26px;"></ons-icon> <span style="float: right; width: calc(100% - 26px);">'+'Please enter your PostCash Secure PIN to proceed</span></div>',cancelable:true,placeholder:"Secure PIN",inputType:"number",defaultValue:"",autofocus:true,submitOnEnter:true});}).then(function(userInput){// user has provided a secured PIN , now authenticate it
if(userInput===utopiasoftware.saveup.model.appUserDetails.securePin){// authentication successful
...
}).catch(function(){});return;}if(label=="transaction history"){// 'transaction history' button was clicked
// close the side menu
$('ons-splitter').get(0).left.close().then(function(){// ask user for secure PIN before proceeding. secure pin MUST match
return ons.notification.prompt({title:"Security Check",id:"pin-security-check",class:"utopiasoftware-no-style",messageHTML:'<div><ons-icon icon="ion-lock-combination" size="24px" '+'style="color: #b388ff; float: left; width: 26px;"></ons-icon> <span style="float: right; width: calc(100% - 26px);">'+'Please enter your PostCash Secure PIN to proceed</span></div>',cancelable:true,placeholder:"Secure PIN",inputType:"number",defaultValue:"",autofocus:true,submitOnEnter:true});}).then(function(userInput){// user has provided a secured PIN , now authenticate it
if(userInput===utopiasoftware.saveup.model.appUserDetails.securePin){// authentication successful
...
$thisPage.get(0).onDeviceBackButton=function(){ons.notification.confirm('Do you want to close the app?',{title:'Exit',buttonLabels:['No','Yes']})// Ask for confirmation
.then(function(index){if(index===1){// OK button
navigator.app.exitApp();// Close the app
}});};// check if the user is currently logged in
if(window.localStorage.getItem("app-status")&&window.localStorage.getItem("app-status")!=""){// user is logged in
// display the user's save phone number on the login page phonenumber input
$('#login-form #user-phone').val(utopiasoftware.saveup.model.appUserDetails.phoneNumber);}// initialise the sign-in form validation
utopiasoftware.saveup.controller.signInPageViewModel.formValidator=$('#login-form').parsley();// attach listener for the sign in button on the sign-in page
$('#login-signin').get(0).onclick=function(){// run the validation method for the sign-in form
utopiasoftware.saveup.controller.signInPageViewModel.formValidator.whenValidate();};// listen for log in form field validation failure event
utopiasoftware.saveup.controller.signInPageViewModel.formValidator.on('field:error',function(fieldInstance){// get the element that triggered the field validation error and use it to display tooltip
// display tooltip
$(fieldInstance.$element).addClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).attr("data-hint",fieldInstance.getErrorsMessages()[0]);});// listen for log in form field validation success event
utopiasoftware.saveup.controller.signInPageViewModel.formValidator.on('field:success',function(fieldInstance){// remove tooltip from element
$(fieldInstance.$element).removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).removeAttr("data-hint");});// listen for log in form validation success
utopiasoftware.saveup.controller.signInPageViewModel.formValidator.on('form:success',utopiasoftware.saveup.controller.signInPageViewModel.signinFormValidated);// hide the loader
$('#loader-modal').get(0).hide();// now that the page is being shown without the loader, start the animation of the icons
$('ons-icon.first,ons-icon.second',$thisPage).addClass('animated swing');// wait for 4 seconds, then stop the pulse animation of the Create Account button
setTimeout(function(){$('#login-create-account',$thisPage).removeClass('pulse');},4000);}},/**
         ...
         * @param event
         */pageHide:function pageHide(event){try{// remove any tooltip being displayed on all forms in the login page
$('#sign-in-page [data-hint]').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$('#sign-in-page [data-hint]').removeAttr("data-hint");// reset the form validator object in the sign-in page
utopiasoftware.saveup.controller.signInPageViewModel.formValidator.reset();}catch(err){}},/**
...
$('#sign-in-page [data-hint]').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$('#sign-in-page [data-hint]').removeAttr("data-hint");// destroy the form validator objects in the login page
utopiasoftware.saveup.controller.signInPageViewModel.formValidator.destroy();}catch(err){}},/**
...
Materialize.toast('Welcome '+utopiasoftware.saveup.model.appUserDetails.firstName,4000);}else{// user cannot sign in authentication failed
$('#loader-modal').get(0).hide();// hide loader
ons.notification.alert({title:"Sign In Failed",messageHTML:'<ons-icon icon="md-close-circle-o" size="30px" '+'style="color: red;"></ons-icon> <span>'+'Invalid Credentials'+'</span>',cancelable:false});}},/**
         * method is triggered when create account button is clicked
         */createAccountButtonClicked:function createAccountButtonClicked(){// move the tab view to the Sign Up tab
$('#login-tabbar').get(0).setActiveTab(1,{animation:"slide"});},/**
         * method is triggered when forgot pin button is clicked
         */forgotPinButtonClicked:function forgotPinButtonClicked(element){// move the tab view to the Reset PIN tab
$('#forgot-pin-popover').get(0).show(element,{});}},/**
     ...
     */createAccountPageViewModel:{/**
         ...
         */pageInit:function pageInit(event){var $thisPage=$(event.target);// get the current page shown
// find all onsen-ui input targets and insert a special class to prevent materialize-css from updating the styles
$('ons-input input',$thisPage).addClass('utopiasoftware-no-style');// call the function used to initialise the app page if the app is fully loaded
loadPageOnAppReady();//function is used to initialise the page if the app is fully ready for execution
function loadPageOnAppReady(){// check to see if onsen is ready and if all app loading has been completed
if(!ons.isReady()||utopiasoftware.saveup.model.isAppReady===false){setTimeout(loadPageOnAppReady,500);// call this function again after half a second
return;}// listen for the back button event
$thisPage.get(0).onDeviceBackButton=function(){// move to the first tab in the tab bar i.e sign-in page
$('#login-tabbar').get(0).setActiveTab(0,{animation:"slide"});};// initialise the create-account form validation
utopiasoftware.saveup.controller.createAccountPageViewModel.formValidator=$('#create-account-form').parsley();// attach listener for the create account button on the create account page
...
Materialize.toast('Sign Up completed! Welcome',4000);}).catch(function(err){if(typeof err!=="string"){// if err is NOT a String
err="Sorry. Sign Up could not be completed";}$('#loader-modal').get(0).hide();// hide loader
ons.notification.alert({title:"Sign Up Failed",messageHTML:'<ons-icon icon="md-close-circle-o" size="30px" '+'style="color: red;"></ons-icon> <span>'+err+'</span>',cancelable:false});});},/**
...
$(buttonElem).css("display","none");// hide the Next button
         ...
         */endButtonClicked:function endButtonClicked(){// load the main menu template
...
if(!ons.isReady()||utopiasoftware.saveup.model.isAppReady===false){setTimeout(loadPageOnAppReady,500);// call this function again after half a second
return;}// listen for the back button event
$('#app-main-navigator').get(0).topPage.onDeviceBackButton=function(){ons.notification.confirm('Do you want to close the app?',{title:'Exit',buttonLabels:['No','Yes']})// Ask for confirmation
.then(function(index){if(index===1){// OK button
navigator.app.exitApp();// Close the app
}});};// hide the loader
$('#loader-modal').get(0).hide();}},/**
...
$('ons-splitter-side').removeAttr("swipeable");},/**
         ...
         *
         * @param label
         */mainMenuButtonsClicked:function mainMenuButtonsClicked(label){if(label=="transfer cash"){// 'transfer cash' button was clicked
...
return;}if(label=="my cards"){// 'my cards' button was clicked
// ask user for secure PIN before proceeding. secure pin MUST match
ons.notification.prompt({title:"Security Check",id:"pin-security-check",class:"utopiasoftware-no-style",messageHTML:'<div><ons-icon icon="ion-lock-combination" size="24px" '+'style="color: #b388ff; float: left; width: 26px;"></ons-icon> <span style="float: right; width: calc(100% - 26px);">'+'Please enter your PostCash Secure PIN to proceed</span></div>',cancelable:true,placeholder:"Secure PIN",inputType:"number",defaultValue:"",autofocus:true,submitOnEnter:true}).then(function(userInput){// user has provided a secured PIN , now authenticate it
if(userInput===utopiasoftware.saveup.model.appUserDetails.securePin){// authentication successful
...
return;}if(label=="transaction history"){// 'transaction history' button was clicked
// ask user for secure PIN before proceeding. secure pin MUST match
ons.notification.prompt({title:"Security Check",id:"pin-security-check",class:"utopiasoftware-no-style",messageHTML:'<div><ons-icon icon="ion-lock-combination" size="24px" '+'style="color: #b388ff; float: left; width: 26px;"></ons-icon> <span style="float: right; width: calc(100% - 26px);">'+'Please enter your PostCash Secure PIN to proceed</span></div>',cancelable:true,placeholder:"Secure PIN",inputType:"number",defaultValue:"",autofocus:true,submitOnEnter:true}).then(function(userInput){// user has provided a secured PIN , now authenticate it
if(userInput===utopiasoftware.saveup.model.appUserDetails.securePin){// authentication successful
...
return;}$('#app-main-navigator').get(0).resetToPage("main-menu-page.html");};// initialise the form validation
utopiasoftware.saveup.controller.verifyAccountPageViewModel.formValidator=$('#verify-account-form').parsley();// attach listener for the verify account button
$('#verify-account-button').get(0).onclick=function(){// run the validation method for the create account form
utopiasoftware.saveup.controller.verifyAccountPageViewModel.formValidator.whenValidate();};// listen for log in form field validation failure event
utopiasoftware.saveup.controller.verifyAccountPageViewModel.formValidator.on('field:error',function(fieldInstance){// get the element that triggered the field validation error and use it to display tooltip
// display tooltip
$(fieldInstance.$element).parent().find('label:eq(0)').addClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).parent().find('label:eq(0)').attr("data-hint",fieldInstance.getErrorsMessages()[0]);});// listen for log in form field validation success event
utopiasoftware.saveup.controller.verifyAccountPageViewModel.formValidator.on('field:success',function(fieldInstance){// remove tooltip from element
$(fieldInstance.$element).parent().find('label:eq(0)').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).parent().find('label:eq(0)').removeAttr("data-hint");});// listen for log in form validation success
utopiasoftware.saveup.controller.verifyAccountPageViewModel.formValidator.on('form:success',utopiasoftware.saveup.controller.verifyAccountPageViewModel.verifyAccountFormValidated);// retrieve the sorted array of banks
...
for(var prop in bankObject){optionTags+='<option value="'+prop+'">'+bankObject[prop]+'</option>';}}$('#verify-account-choose-bank',$thisPage).append(optionTags);// append all the created option tags
// initilise the select element
$('#verify-account-choose-bank',$thisPage).material_select();// initialise the character counter plugin
$('#verify-account-number',$thisPage).characterCounter();// remove the progress indeterminate loader
$('.progress',$thisPage).remove();// make the verify account form visible
$('#verify-account-form',$thisPage).css("display","block");// enable the 'Verify Account' button
$('#verify-account-button',$thisPage).removeAttr("disabled");// hide the loader
$('#loader-modal').get(0).hide();}).catch();}},/**
         * method is triggered when the create-account page is hidden
         * @param event
         */pageHide:function pageHide(event){try{// remove any tooltip being displayed on all forms on the page
$('#verify-account-page [data-hint]').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$('#verify-account-page [data-hint]').removeAttr("data-hint");// reset the form validator object on the page
utopiasoftware.saveup.controller.verifyAccountPageViewModel.formValidator.reset();}catch(err){}},/**
         * method is triggered when the sign-in page is destroyed
         * @param event
         */pageDestroy:function pageDestroy(event){try{// remove any tooltip being displayed on all forms on the page
$('#verify-account-page [data-hint]').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$('#verify-account-page [data-hint]').removeAttr("data-hint");// destroy the form validator objects on the page
utopiasoftware.saveup.controller.verifyAccountPageViewModel.formValidator.destroy();// destroy the form inputs which need to be destroyed
$('#verify-account-choose-bank').material_select('destroy');$('#verify-account-number').off();$('#verify-account-number').removeData();}catch(err){}},/**
         * method is triggered when verify account form is successfully validated
         */verifyAccountFormValidated:function verifyAccountFormValidated(){// check if Internet Connection is available before proceeding
if(navigator.connection.type===Connection.NONE){// no Internet Connection
// inform the user that they cannot proceed without Internet
window.plugins.toast.showWithOptions({message:"You cannot verify accounts without an Internet Connection",duration:4000,position:"top",styling:{opacity:1,backgroundColor:'#ff0000',//red
textColor:'#FFFFFF',textSize:14}},function(toastEvent){if(toastEvent&&toastEvent.event=="touch"){// user tapped the toast, so hide toast immediately
...
$('#loader-modal-message').html("Verifying Bank Account...");$('#loader-modal').get(0).show();// show loader
// request for gateway authorization token
utopiasoftware.saveup.moneyWaveObject.useToken.then(function(tokenData){// verify the bank account
return new Promise(function(resolve,reject){var verifyAccountRequest=$.ajax({url:utopiasoftware.saveup.moneyWaveObject.gateway+"v1/resolve/account",type:"post",contentType:"application/json",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("Authorization",tokenData);},dataType:"json",timeout:240000,// wait for 4 minutes before timeout of request
processData:false,data:JSON.stringify({account_number:$('#verify-account-form #verify-account-number').val(),bank_code:$('#verify-account-form #verify-account-choose-bank').val()})});// server responded to account verification request
verifyAccountRequest.done(function(responseData){if(responseData.status==="success"){// the server responded with a successful account verification
// set the name of the account which has been verified
$('#verify-account-form #verify-account-name').val(responseData.data.account_name);resolve();// resolve the account verification promise
...
verifyAccountRequest.fail(function(){reject();// reject the account verification promise
});});}).then(function(){// hide the loader modal
return $('#loader-modal').get(0).hide();// hide loader
}).then(function(){// show a toast welcoming user
Materialize.toast('Bank account verified <ons-button modifier="quiet" style="color: #82b1ff" onclick="utopiasoftware.saveup.controller.verifyAccountPageViewModel.moreActions();">More</ons-button>',4000);// show the result of the verification to the user
$('#verify-account-form .verify-account-success').css("display","block");$('#verify-account-form .verify-account-success').addClass("scale-in");}).catch(function(){// inform the user that this is an invalid account number
// hide the loader modal
$('#loader-modal').get(0).hide();// hide loader
// show the FAILED result of the verification to the user
$('#verify-account-form .verify-account-failed').css("display","block");$('#verify-account-form .verify-account-failed').addClass("scale-in");});},/**
         * method is used to specifically reset the verified account display used
         * when an account has been previously verified
         *
         */resetVerifiedAcctDisplay:function resetVerifiedAcctDisplay(){// hide all account verification information
$('#verify-account-form .verify-account-success').css("display","none");$('#verify-account-form .verify-account-success').removeClass("scale-in");$('#verify-account-form .verify-account-failed').css("display","none");$('#verify-account-form .verify-account-failed').removeClass("scale-in");},/**
         * method is used to display the means for performing more actions upon/after
         * bank account verification
         */moreActions:function moreActions(){// display the bottom sheets
...
$('#verify-account-bottom-sheet').modal('close');$('#transfer-cash-page').remove();// remove previous transfer cash pages
// call the transfer-cash=page and pass the verified account as recipient details data
$('#app-main-navigator').get(0).pushPage("transfer-cash-page.html",{data:{recipient_details:{recipientAccountName:$('#verify-account-name').val(),recipientAccount:$('#verify-account-page #verify-account-number').val(),recipientBankCode:$('#verify-account-page #verify-account-choose-bank').val()}}});// navigate to the specified page
}else{// inform user that security check failed/user authentication failed
...
if($('ons-splitter').get(0).left.isOpen){// side menu open, so close it
$('ons-splitter').get(0).left.close();return;// exit the method
}$('#app-main-navigator').get(0).resetToPage("main-menu-page.html");};// register listener for the pull-to-refresh widget
$('#my-cards-pull-hook',$thisPage).on("changestate",function(event){// check the state of the pull-to-refresh widget
switch(event.originalEvent.state){case'initial':// update the displayed icon
$('#my-cards-pull-hook-fab',event.originalEvent.pullHook).html('<ons-icon icon="fa-long-arrow-down" size="24px"></ons-icon>');break;case'preaction':$('#my-cards-pull-hook-fab',event.originalEvent.pullHook).html('<ons-icon icon="fa-long-arrow-up" size="24px"></ons-icon>');break;case'action':$('#my-cards-pull-hook-fab',event.originalEvent.pullHook).html('<ons-icon icon="fa-repeat" size="24px" spin></ons-icon>');break;}});// add method to handle the loading action of the pull-to-refresh widget
$('#my-cards-pull-hook',$thisPage).get(0).onAction=function(loadingDone){// disable pull-to-refresh widget till loading is done
$('#my-cards-pull-hook',$thisPage).attr("disabled",true);// load the card data from the device secure store
utopiasoftware.saveup.financialCardOperations.loadCardData().then(function(cardsArray){// the cards array collection has been returned
...
$('#my-cards-page-error',$thisPage).css("display","none");// hide the my-cards-list from display
$('#my-cards-list',$thisPage).css("display","none");// enable the 'Add Card' button
$('#my-cards-add-card-button',$thisPage).removeAttr("disabled");}else{// there are card data available
// empty the contents of the my cards list
$('#my-cards-list',$thisPage).html("");for(var index=0;index<cardsArray.length;index++){// append the stored cards to the "My Cards" list
// create the card content
var cardContent='<div class="row"><div class="col s1"></div><div class="col s10">\n                            <div class="card small">\n                            <div class="card-image" style="padding-top: 3%; padding-left: 3%; padding-right: 3%;\n                            background-image: url(\''+cardsArray[index].cardImage+'\'); background-position: center; background-size: contain; background-origin: content-box; background-repeat: no-repeat;">\n                            <img src="css/app-images/blank.png">\n                            </div>\n                            <div class="card-content" style="padding-bottom: 5px;">\n                            <div style="font-weight: bold; font-style: italic; color: #464646">'+cardsArray[index].cardNickName+'</div>\n                            <div style="font-weight: bold; font-size: 0.8em; color: #464646">'+cardsArray[index].cardBrand+'\n                            '+(cardsArray[index].cardLocale=="international"?"(International)":"")+'</div>\n                            </div>\n                            <div class="card-action" style="padding: 0;">\n                            <ons-button data-id="'+cardsArray[index].cardUniqueId+'"\n                                data-acct-object=\''+JSON.stringify({senderCardNickName:cardsArray[index].cardNickName,senderCardNumber:cardsArray[index].cardNumber,senderCardCvv:cardsArray[index].cvv,senderCardExpiryMonth:cardsArray[index].cardExpiryMonth,senderCardExpiryYear:cardsArray[index].cardExpiryYear})+'\' modifier="quiet" disable-auto-styling class="right"\n                                        style="color: #464646; padding:0; margin-top: 0.5em; margin-left: 1em; margin-right: 1em;"\n                                        onclick="utopiasoftware.saveup.controller.myCardsPageViewModel.transferCashButtonClicked(this);">\n                                <ons-icon icon="md-saveup-icon-saveup-transfer-cash" size="29px">\n                                </ons-icon>\n                                </ons-button>\n                            <ons-button data-id="'+cardsArray[index].cardUniqueId+'" modifier="quiet" disable-auto-styling class="right"\n                                    style="color: #464646; padding:0; margin-top: 0.5em; margin-left: 1em;"\n                                    onclick="utopiasoftware.saveup.controller.myCardsPageViewModel.editCardButtonClicked(this)">\n                            <ons-icon icon="md-edit" size="25px">\n                            </ons-icon>\n                            </ons-button>\n                            <ons-button data-id="'+cardsArray[index].cardUniqueId+'" modifier="quiet" disable-auto-styling class="right"\n                                    style="color: #464646; padding:0; margin-top: 0.5em; margin-left: 1em;"\n                                    onclick="utopiasoftware.saveup.controller.myCardsPageViewModel.deleteCardButtonClicked(this)">\n                            <ons-icon icon="md-delete" size="25px">\n                            </ons-icon>\n                            </ons-button>\n                            </div>\n                            </div>\n                            </div>\n                            <div class="col s1"></div>\n                            </div>';// append the card content to the "My Card" list
...
$('#my-cards-page-error',$thisPage).css("display","none");// display the my-cards-list
$('#my-cards-list',$thisPage).css("display","block");// enable the 'Add Card' button
$('#my-cards-add-card-button',$thisPage).removeAttr("disabled");}}).catch(function(){// an error occurred, so display the error message to the user
// remove the page preloader progress bar
$('.progress',$thisPage).remove();// display the help button
$('#my-cards-help-1',$thisPage).css("display","inline-block");// enable the pull-to-refresh widget for the page
$('#my-cards-pull-hook',$thisPage).removeAttr("disabled");// hide a message to inform user that there are no cards available
...
$('#my-cards-page-error',$thisPage).css("display","block");// hide the my-cards-list from display
$('#my-cards-list',$thisPage).css("display","none");// disable the 'Add Card' button
$('#my-cards-add-card-button',$thisPage).attr("disabled",true);});// hide the loader
$('#loader-modal').get(0).hide();}},/**
         * method is triggered when page is shown
         *
         * @param event
...
window.plugins.toast.hide();}});});}});},/**
         ...
         *
         * @param buttonElem
         */transferCashButtonClicked:function transferCashButtonClicked(buttonElem){// ask user for secure PIN before proceeding. secure pin MUST match
ons.notification.prompt({title:"Security Check",id:"pin-security-check",class:"utopiasoftware-no-style",messageHTML:'<div><ons-icon icon="ion-lock-combination" size="24px" '+'style="color: #b388ff; float: left; width: 26px;"></ons-icon> <span style="float: right; width: calc(100% - 26px);">'+'Please enter your PostCash Secure PIN to proceed</span></div>',cancelable:true,placeholder:"Secure PIN",inputType:"number",defaultValue:"",autofocus:true,submitOnEnter:true}).then(function(userInput){// user has provided a secured PIN , now authenticate it
if(userInput===utopiasoftware.saveup.model.appUserDetails.securePin){// authentication successful
// close the 'verify account' the bottom sheets
$('#verify-account-bottom-sheet').modal('close');$('#transfer-cash-page').remove();// remove previous transfer cash pages
// call the transfer-cash=page and pass the 'my card' details contained in the data attribute as card details data
$('#app-main-navigator').get(0).pushPage("transfer-cash-page.html",{data:{sender_card_details:JSON.parse($(buttonElem).attr("data-acct-object"))}});// navigate to the specified page
}else{// inform user that security check failed/user authentication failed
ons.notification.alert({title:"Security Check",messageHTML:'<ons-icon icon="md-close-circle-o" size="30px" '+'style="color: red;"></ons-icon> <span>'+'Security check failed. Invalid credentials'+'</span>',cancelable:true});}}).catch(function(){});return;}},/**
     ...
     */addCardPageViewModel:{/**
         ...
         */newCardImage:"",cardImageRandomNum:null,/**
         * event is triggered when page is initialised
         */pageInit:function pageInit(event){var $thisPage=$(event.target);// get the current page shown
// find all onsen-ui input targets and insert a special class to prevent materialize-css from updating the styles
$('ons-input input',$thisPage).addClass('utopiasoftware-no-style');// disable the swipeable feature for the app splitter
$('ons-splitter-side').removeAttr("swipeable",true);// reset the previous & current scroll positions of the page contents
utopiasoftware.saveup.controller.addCardPageViewModel.previousScrollPosition=0;utopiasoftware.saveup.controller.addCardPageViewModel.currentScrollPosition=0;// reset the new card brand, card general locale & card image
utopiasoftware.saveup.controller.addCardPageViewModel.newCardBrand="Unknown";utopiasoftware.saveup.controller.addCardPageViewModel.newCardLocale="Unknown";utopiasoftware.saveup.controller.addCardPageViewModel.newCardImage="";// generate the random number used to display the financial card image
var randomGen=new Random(Random.engines.nativeMath);// random number generator
// generate the random number
utopiasoftware.saveup.controller.addCardPageViewModel.cardImageRandomNum=randomGen.integer(1,3);// call the function used to initialise the app page if the app is fully loaded
loadPageOnAppReady();//function is used to initialise the page if the app is fully ready for execution
function loadPageOnAppReady(){// check to see if onsen is ready and if all app loading has been completed
if(!ons.isReady()||utopiasoftware.saveup.model.isAppReady===false){setTimeout(loadPageOnAppReady,500);// call this function again after half a second
return;}// listen for the back button event
...
$('#add-card-page .page__content').on("scroll",utopiasoftware.saveup.controller.addCardPageViewModel.pageContentScrolled);// initialise the form validation
utopiasoftware.saveup.controller.addCardPageViewModel.formValidator=$('#add-card-form').parsley();// attach listener for the 'save' button click
$('#add-card-save-button').get(0).onclick=function(){// run the validation method for the create account form
utopiasoftware.saveup.controller.addCardPageViewModel.formValidator.whenValidate();};// listen for form field validation failure event
utopiasoftware.saveup.controller.addCardPageViewModel.formValidator.on('field:ajaxoptions',function(fieldInstance,ajaxOptions){// edit the ajax options object to include the necessary authorization header
ajaxOptions.headers={"Authorization":"Bearer "+utopiasoftware.saveup.paystackObject.key.secret};// display the loading icon for card number validation
$('#add-card-number-validation-loading',$thisPage).css("display","block");});// listen for form field validation failure event
utopiasoftware.saveup.controller.addCardPageViewModel.formValidator.on('field:error',function(fieldInstance){// get the element that triggered the field validation error and use it to display tooltip
// display tooltip
$(fieldInstance.$element).parent().find('label:eq(0)').addClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).parent().find('label:eq(0)').attr("data-hint",fieldInstance.getErrorsMessages()[0]);// check if the element that triggered that validation error was the card number input
if($(fieldInstance.$element).is('#add-card-card-number')){// it is the card number input
// hide the loading icon
$('#add-card-number-validation-loading',$thisPage).css("display","none");// hide the card image row
$('#add-card-image-container',$thisPage).css("display","none");}});// listen for form field validation success event
utopiasoftware.saveup.controller.addCardPageViewModel.formValidator.on('field:success',function(fieldInstance){// remove tooltip from element
$(fieldInstance.$element).parent().find('label:eq(0)').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).parent().find('label:eq(0)').removeAttr("data-hint");// check if the element that triggered that validation success was the card number input
if($(fieldInstance.$element).is('#add-card-card-number')){// it is the card number input
// hide the loading icon
$('#add-card-number-validation-loading',$thisPage).css("display","none");// check if the user request for remote card number validation
...
switch(utopiasoftware.saveup.controller.addCardPageViewModel.newCardBrand){case"Mastercard":if(utopiasoftware.saveup.controller.addCardPageViewModel.newCardLocale=="local"){// this is a local mastercard
// set the image for the card
utopiasoftware.saveup.controller.addCardPageViewModel.newCardImage="css/app-images/mastercard-local-"+utopiasoftware.saveup.controller.addCardPageViewModel.cardImageRandomNum+".png";$('#add-card-image').attr("src","css/app-images/mastercard-local-"+utopiasoftware.saveup.controller.addCardPageViewModel.cardImageRandomNum+".png");// update the card type display name
$('#add-card-image-type').html("Mastercard");}else{// this is a international mastercard
// set the image for the card
utopiasoftware.saveup.controller.addCardPageViewModel.newCardImage="css/app-images/mastercard-international-"+utopiasoftware.saveup.controller.addCardPageViewModel.cardImageRandomNum+".png";$('#add-card-image').attr("src","css/app-images/mastercard-international-"+utopiasoftware.saveup.controller.addCardPageViewModel.cardImageRandomNum+".png");// update the card type display name
$('#add-card-image-type').html("Mastercard (International)");}break;case"Visa":if(utopiasoftware.saveup.controller.addCardPageViewModel.newCardLocale=="local"){// this is a local visa
// set the image for the card
utopiasoftware.saveup.controller.addCardPageViewModel.newCardImage="css/app-images/visacard-local-"+utopiasoftware.saveup.controller.addCardPageViewModel.cardImageRandomNum+".png";$('#add-card-image').attr("src","css/app-images/visacard-local-"+utopiasoftware.saveup.controller.addCardPageViewModel.cardImageRandomNum+".png");// update the card type display name
$('#add-card-image-type').html("Visa");}else{// this is a international Visa card
// set the image for the card
utopiasoftware.saveup.controller.addCardPageViewModel.newCardImage="css/app-images/visacard-international-"+utopiasoftware.saveup.controller.addCardPageViewModel.cardImageRandomNum+".png";$('#add-card-image').attr("src","css/app-images/visacard-international-"+utopiasoftware.saveup.controller.addCardPageViewModel.cardImageRandomNum+".png");// update the card type display name
$('#add-card-image-type').html("Visa (International)");}break;case"Verve":if(utopiasoftware.saveup.controller.addCardPageViewModel.newCardLocale=="local"){// this is a local verve
// set the image for the card
utopiasoftware.saveup.controller.addCardPageViewModel.newCardImage="css/app-images/verve-local-"+utopiasoftware.saveup.controller.addCardPageViewModel.cardImageRandomNum+".png";$('#add-card-image').attr("src","css/app-images/verve-local-"+utopiasoftware.saveup.controller.addCardPageViewModel.cardImageRandomNum+".png");// update the card type display name
$('#add-card-image-type').html("Verve");}break;}// display the card image row
$('#add-card-image-container',$thisPage).css("display","block");}else{// user did not ask for remote card validation
// set the image for the card
utopiasoftware.saveup.controller.addCardPageViewModel.newCardImage="css/app-images/unknown-local-"+utopiasoftware.saveup.controller.addCardPageViewModel.cardImageRandomNum+".png";// set new card brand as unknown
utopiasoftware.saveup.controller.addCardPageViewModel.newCardBrand="Unknown";// set the new card locale as unknown
utopiasoftware.saveup.controller.addCardPageViewModel.newCardLocale="Unknown";// hide the card image row
$('#add-card-image-container',$thisPage).css("display","none");}}});// listen for form validation success
utopiasoftware.saveup.controller.addCardPageViewModel.formValidator.on('form:success',utopiasoftware.saveup.controller.addCardPageViewModel.addCardFormValidated);/** dynamically create the contents of the various select elements **/var optionTags="";// string to hold all created option tags for the Card Expiry Year
...
optionTags+='<option value="'+yearOption+'">'+yearOption+'</option>';}$('#add-card-expiry-year',$thisPage).append(optionTags);// append all the created option tags
// initialise all the select element
$('select',$thisPage).material_select();// initialise the character counter plugin
...
$('.progress',$thisPage).remove();// make the add card form visible
$('#add-card-form',$thisPage).css("display","block");// enable the 'Cancel' & 'Save' buttons
$('#add-card-cancel-button, #add-card-save-button',$thisPage).removeAttr("disabled");// hide the loader
...
utopiasoftware.saveup.controller.addCardPageViewModel.currentScrollPosition=$(this).scrollTop();if(utopiasoftware.saveup.controller.addCardPageViewModel.currentScrollPosition>utopiasoftware.saveup.controller.addCardPageViewModel.previousScrollPosition){// user scrolled up
// set the current position as previous position
utopiasoftware.saveup.controller.addCardPageViewModel.previousScrollPosition=utopiasoftware.saveup.controller.addCardPageViewModel.currentScrollPosition;// check if the header image left after scrolling is <= height of page toolbar
if(140-utopiasoftware.saveup.controller.addCardPageViewModel.currentScrollPosition<=56){// the header image left after scrolling is <= height of page toolbar
...
$('#add-card-page ons-list-header').css({"display":"block","position":"fixed","top":"56px","width":"100%"});this.isToolBarOpaque=true;// flag that toolbar has been made opaque
}}return;}if(utopiasoftware.saveup.controller.addCardPageViewModel.currentScrollPosition<utopiasoftware.saveup.controller.addCardPageViewModel.previousScrollPosition){// user scrolled down
// set the current position as previous position
utopiasoftware.saveup.controller.addCardPageViewModel.previousScrollPosition=utopiasoftware.saveup.controller.addCardPageViewModel.currentScrollPosition;// check if the header image left after scrolling is > height of page toolbar
if(140-utopiasoftware.saveup.controller.addCardPageViewModel.currentScrollPosition>56){// the header image left after scrolling is > height of page toolbar
...
$('#add-card-page ons-list-header').css({"display":"block","position":"static","top":"56px"});this.isToolBarOpaque=false;// flag that toolbar has been made transparent
}}return;}},/**
         * custom parsley validator for financial cards (including visa, master, verve)
         *
         * @param jqxhr {jqueryXhr}
         */financialCardValidator:function financialCardValidator(jqxhr){var serverResponse="";// holds the server response
// check the validator response
if(jqxhr.status!=200){// request was NOT success
// set new card brand as unknown
utopiasoftware.saveup.controller.addCardPageViewModel.newCardBrand="Unknown";// set the new card locale as unknown
utopiasoftware.saveup.controller.addCardPageViewModel.newCardLocale="Unknown";return false;}// convert the server response to json
serverResponse=JSON.parse(jqxhr.responseText.trim());if(serverResponse.status!=true){// the server api response was NOT successful
// set new card brand as unknown
utopiasoftware.saveup.controller.addCardPageViewModel.newCardBrand="Unknown";// set the new card locale as unknown
utopiasoftware.saveup.controller.addCardPageViewModel.newCardLocale="Unknown";return false;}if(serverResponse.data.brand==""||serverResponse.data.brand=="Unknown"){// card could not be identified
// set new card brand as unknown
utopiasoftware.saveup.controller.addCardPageViewModel.newCardBrand="Unknown";// set the new card locale as unknown
utopiasoftware.saveup.controller.addCardPageViewModel.newCardLocale="Unknown";return false;}// get the brand of the new card
if(serverResponse.data.brand.toLocaleUpperCase().indexOf("MASTER")>=0){// brand is a Mastercard
// set new card brand as mastercard
utopiasoftware.saveup.controller.addCardPageViewModel.newCardBrand="Mastercard";}if(serverResponse.data.brand.toLocaleUpperCase().indexOf("VISA")>=0){// brand is a Visa
// set new card brand as visa
utopiasoftware.saveup.controller.addCardPageViewModel.newCardBrand="Visa";}if(serverResponse.data.brand.toLocaleUpperCase().indexOf("VERVE")>=0){// brand is a Verve
// set new card brand as verve
utopiasoftware.saveup.controller.addCardPageViewModel.newCardBrand="Verve";}// get the general locale of the new card
if(serverResponse.data.country_name.indexOf("Nigeria")>=0){// card is local
// set the new card locale
utopiasoftware.saveup.controller.addCardPageViewModel.newCardLocale="local";}if(serverResponse.data.country_name.indexOf("Nigeria")<0){// card is international
// set the new card locale
utopiasoftware.saveup.controller.addCardPageViewModel.newCardLocale="international";}return true;// validation successful
}},/**
...
$('#my-accounts-pull-hook',$thisPage).on("changestate",function(event){// check the state of the pull-to-refresh widget
switch(event.originalEvent.state){case'initial':// update the displayed icon
$('#my-accounts-pull-hook-fab',event.originalEvent.pullHook).html('<ons-icon icon="fa-long-arrow-down" size="24px"></ons-icon>');break;case'preaction':$('#my-accounts-pull-hook-fab',event.originalEvent.pullHook).html('<ons-icon icon="fa-long-arrow-up" size="24px"></ons-icon>');break;case'action':$('#my-accounts-pull-hook-fab',event.originalEvent.pullHook).html('<ons-icon icon="fa-repeat" size="24px" spin></ons-icon>');break;}});// add method to handle the loading action of the pull-to-refresh widget
$('#my-accounts-pull-hook',$thisPage).get(0).onAction=function(loadingDone){// disable pull-to-refresh widget till loading is done
$('#my-accounts-pull-hook',$thisPage).attr("disabled",true);// load the user's bank account data from the device secure store
...
// inform the user that card was deleted
Materialize.toast('Bank Account deleted',3000);}).catch(function(){// inform the user that the specified bank account could not be deleted
window.plugins.toast.showWithOptions({message:"Sorry, the bank account could not be deleted.\n Try again",duration:4000,position:"top",styling:{opacity:1,backgroundColor:'#ff0000',//red
textColor:'#FFFFFF',textSize:14}},function(toastEvent){if(toastEvent&&toastEvent.event=="touch"){// user tapped the toast, so hide toast immediately
window.plugins.toast.hide();}});});}});},/**
...
$('#app-main-navigator').get(0).pushPage("transfer-cash-page.html",{data:{sender_details:JSON.parse($(buttonElem).attr("data-acct-object"))}});// navigate to the specified page
}else{// inform user that security check failed/user authentication failed
ons.notification.alert({title:"Security Check",messageHTML:'<ons-icon icon="md-close-circle-o" size="30px" '+'style="color: red;"></ons-icon> <span>'+'Security check failed. Invalid credentials'+'</span>',cancelable:true});}}).catch(function(){});return;}},/**
...
$('#loader-modal #loader-modal-message').html("Preparing Device Storage...");$('#loader-modal').get(0).show();// request for gateway authorization token used in account number validation
...
utopiasoftware.saveup.controller.addAccountPageViewModel.tokenData=tokenData;// hide loader message
$('#loader-modal').get(0).hide();// run the validation method for the create account form
...
utopiasoftware.saveup.controller.addAccountPageViewModel.tokenData="";// hide loader message
$('#loader-modal').get(0).hide();// run the validation method for the create account form
...
$('#add-account-page #add-account-form').get(0).reset();$('#add-account-page select').material_select();// reset the account number validation check and all its related actions
utopiasoftware.saveup.controller.addAccountPageViewModel.isbankAccountNumberValidated($('#add-account-page #add-account-verify-account'));// reset the bank account properties
...
$('#saved-recipients-pull-hook',$thisPage).on("changestate",function(event){// check the state of the pull-to-refresh widget
switch(event.originalEvent.state){case'initial':// update the displayed icon
$('#saved-recipients-pull-hook-fab',event.originalEvent.pullHook).html('<ons-icon icon="fa-long-arrow-down" size="24px"></ons-icon>');break;case'preaction':$('#saved-recipients-pull-hook-fab',event.originalEvent.pullHook).html('<ons-icon icon="fa-long-arrow-up" size="24px"></ons-icon>');break;case'action':$('#saved-recipients-pull-hook-fab',event.originalEvent.pullHook).html('<ons-icon icon="fa-repeat" size="24px" spin></ons-icon>');break;}});// add method to handle the loading action of the pull-to-refresh widget
$('#saved-recipients-pull-hook',$thisPage).get(0).onAction=function(loadingDone){// disable pull-to-refresh widget till loading is done
$('#saved-recipients-pull-hook',$thisPage).attr("disabled",true);// load the saved recipients account data from the device secure store
...
ons.notification.confirm("Do you want to delete this recipient's bank account?",{title:'Confirm Delete',buttonLabels:['No','Yes']})// Ask for confirmation
.then(function(index){if(index===1){// YES button clicked
// call the utility method used to delete a specified recipient bank account
utopiasoftware.saveup.savedRecipientsBankAccountOperations.deleteSavedRecipientAccount($(buttonElem).attr("data-id")).then(function(){// account has been deleted
$(buttonElem).closest('.row').remove();// remove the account from display
// inform the user that card was deleted
Materialize.toast("Recipient's Bank Account deleted",3000);}).catch(function(){// inform the user that the specified bank account could not be deleted
window.plugins.toast.showWithOptions({message:"Sorry, the recipient's bank account could not be deleted.\n Try again",duration:4000,position:"top",styling:{opacity:1,backgroundColor:'#ff0000',//red
textColor:'#FFFFFF',textSize:14}},function(toastEvent){if(toastEvent&&toastEvent.event=="touch"){// user tapped the toast, so hide toast immediately
window.plugins.toast.hide();}});});}});},/**
         * method is used to trigger the edit operation of a user's bank account
         * and updating/changing the user-interface (UI)
...
$('#app-main-navigator').get(0).pushPage("transfer-cash-page.html",{data:{recipient_details:JSON.parse($(buttonElem).attr("data-acct-object"))}});// navigate to the specified page
}else{// inform user that security check failed/user authentication failed
ons.notification.alert({title:"Security Check",messageHTML:'<ons-icon icon="md-close-circle-o" size="30px" '+'style="color: red;"></ons-icon> <span>'+'Security check failed. Invalid credentials'+'</span>',cancelable:true});}}).catch(function(){});return;}},/**
...
$('ons-input input',$thisPage).addClass('utopiasoftware-no-style');// disable the swipeable feature for the app splitter
$('ons-splitter-side').removeAttr("swipeable",true);// reset the previous & current scroll positions of the page contents
utopiasoftware.saveup.controller.addRecipientPageViewModel.previousScrollPosition=0;utopiasoftware.saveup.controller.addRecipientPageViewModel.currentScrollPosition=0;// reset the bank account properties
utopiasoftware.saveup.controller.addRecipientPageViewModel.bankAccountName="";utopiasoftware.saveup.controller.addRecipientPageViewModel.tokenData="";utopiasoftware.saveup.controller.addRecipientPageViewModel.bankAccountImage="";// generate the random number used to display the bank account image/avatar
var randomGen=new Random(Random.engines.nativeMath);// random number generator
// generate the random number
utopiasoftware.saveup.controller.addRecipientPageViewModel.bankAccountImageRandomNum=randomGen.integer(1,6);// call the function used to initialise the app page if the app is fully loaded
loadPageOnAppReady();//function is used to initialise the page if the app is fully ready for execution
function loadPageOnAppReady(){// check to see if onsen is ready and if all app loading has been completed
if(!ons.isReady()||utopiasoftware.saveup.model.isAppReady===false){setTimeout(loadPageOnAppReady,500);// call this function again after half a second
return;}// listen for the back button event
...
$('#add-recipient-page .page__content').on("scroll",utopiasoftware.saveup.controller.addRecipientPageViewModel.pageContentScrolled);// initialise the form validation
utopiasoftware.saveup.controller.addRecipientPageViewModel.formValidator=$('#add-recipient-form').parsley();// attach listener for the 'save' button click
$('#add-recipient-save-button').get(0).onclick=function(){// check if the user requested for remote account number validation
if($('#add-recipient-account-verify-account').is(":checked")){//user asked for remote validation
// display loader message to the user
$('#loader-modal #loader-modal-message').html("Preparing Device Storage...");$('#loader-modal').get(0).show();// request for gateway authorization token used in account number validation
utopiasoftware.saveup.moneyWaveObject.useToken.then(function(tokenData){// set the token data used for verifying the bank account number
utopiasoftware.saveup.controller.addRecipientPageViewModel.tokenData=tokenData;// hide loader message
$('#loader-modal').get(0).hide();// run the validation method for the form
utopiasoftware.saveup.controller.addRecipientPageViewModel.formValidator.whenValidate();}).catch(function(){// clear the token data used for verifying the bank account number
utopiasoftware.saveup.controller.addRecipientPageViewModel.tokenData="";// hide loader message
$('#loader-modal').get(0).hide();// run the validation method for the form
utopiasoftware.saveup.controller.addRecipientPageViewModel.formValidator.whenValidate();});}else{// user did not ask for remote account number validation
// run the validation method for the form
utopiasoftware.saveup.controller.addRecipientPageViewModel.formValidator.whenValidate();}};// listen for form field validation failure event
utopiasoftware.saveup.controller.addRecipientPageViewModel.formValidator.on('field:ajaxoptions',function(fieldInstance,ajaxOptions){// edit the ajax options object to include the necessary authorization header
ajaxOptions.headers={"Authorization":utopiasoftware.saveup.controller.addRecipientPageViewModel.tokenData};ajaxOptions.data=JSON.stringify({account_number:$('#add-recipient-form #add-recipient-account-number').val(),bank_code:$('#add-recipient-form #add-recipient-choose-bank').val()});// display the loading icon for account number validation
$('#add-recipient-account-number-validation-loading',$thisPage).css("display","block");});// listen for form field validation failure event
utopiasoftware.saveup.controller.addRecipientPageViewModel.formValidator.on('field:error',function(fieldInstance){// get the element that triggered the field validation error and use it to display tooltip
// display tooltip
$(fieldInstance.$element).parent().find('label:eq(0)').addClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).parent().find('label:eq(0)').attr("data-hint",fieldInstance.getErrorsMessages()[0]);// check if the element that triggered that validation error was the account number input
...
if($('#add-recipient-account-verify-account').is(":checked")){//user asked for remote validation
// empty the content of the account name input (based on remote validation result)
$('#add-recipient-page #add-recipient-account-name').val("");}else{// user did not ask for remote account number validation
}}});// listen for form field validation success event
utopiasoftware.saveup.controller.addRecipientPageViewModel.formValidator.on('field:success',function(fieldInstance){// remove tooltip from element
$(fieldInstance.$element).parent().find('label:eq(0)').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).parent().find('label:eq(0)').removeAttr("data-hint");// check if the element that triggered that validation success was the account number input
if($(fieldInstance.$element).is('#add-recipient-account-number')){// it is the account number input
// hide the loading icon
$('#add-recipient-account-number-validation-loading',$thisPage).css("display","none");// check if the user requested for remote account number validation
...
for(var index=0;index<bankArrayData.length;index++){var bankObject=bankArrayData[index];// get the bank object
// update the banks select element option tags
for(var prop in bankObject){optionTags+='<option value="'+prop+'">'+bankObject[prop]+'</option>';}}$('#add-recipient-choose-bank',$thisPage).append(optionTags);// append all the created option tags
// initialise the select element
$('#add-recipient-choose-bank',$thisPage).material_select();return;// return at this point, so the page can continue initialisation
}).then(function(){// initialise the character counter plugin
$('#add-recipient-account-number',$thisPage).characterCounter();return;}).then(function(){// check if the page was sent a bank account id.
// if so preload the bank account data into the form
if($('#app-main-navigator').get(0).topPage.data&&$('#app-main-navigator').get(0).topPage.data.edit){// get the details of the account to be edited
utopiasoftware.saveup.savedRecipientsBankAccountOperations.getSavedRecipientAccount($('#app-main-navigator').get(0).topPage.data.edit).then(function(bankAcct){$('#add-recipient-page #add-recipient-unique-id').val(bankAcct.uniqueAccountId);$('#add-recipient-page #add-recipient-account-number').val(bankAcct.bankAccountNumber);$('#add-recipient-page #add-recipient-account-name').val(bankAcct.bankAccountName);$('#add-recipient-page #add-recipient-choose-bank').val(bankAcct.flutterwave_bankCode);$('#add-recipient-page #hidden-choose-bank-input').val(bankAcct.flutterwave_bankCode);// re-update the form input fields
Materialize.updateTextFields();//re-initialise the form select elements
$('select',$thisPage).material_select();// remove the progress indeterminate loader
$('.progress',$thisPage).remove();// make the add recipient form visible
$('#add-recipient-form',$thisPage).css("display","block");// enable the 'Cancel' & 'Save' buttons
$('#add-recipient-cancel-button, #add-recipient-save-button',$thisPage).removeAttr("disabled");// hide the loader
$('#loader-modal').get(0).hide();});}else{// this is not an edit operation, but a CREATE operation
...
$('#loader-modal').get(0).hide();}}).catch();}},/**
         ...
         * @param event
         */pageHide:function pageHide(event){try{// remove any tooltip being displayed on all forms on the page
$('#add-recipient-page [data-hint]').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$('#add-recipient-page [data-hint]').removeAttr("data-hint");// reset the form validator object on the page
utopiasoftware.saveup.controller.addRecipientPageViewModel.formValidator.reset();}catch(err){}},/**
         * method is triggered when the page is destroyed
         * @param event
         */pageDestroy:function pageDestroy(event){try{// remove any tooltip being displayed on all forms on the page
$('#add-recipient-page [data-hint]').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$('#add-recipient-page [data-hint]').removeAttr("data-hint");// destroy the form validator objects on the page
utopiasoftware.saveup.controller.addRecipientPageViewModel.formValidator.destroy();// destroy the form inputs which need to be destroyed
$('#add-recipient-page select').material_select('destroy');$('#add-recipient-page #add-recipient-account-number').off();$('#add-recipient-page #add-recipient-account-number').removeData();}catch(err){}},/**
         * method is triggered when add card form is successfully validated
         */addRecipientFormValidated:function addRecipientFormValidated(){// display the secure storage modal to indicate that Recipient's Bank Account is being securely stored
$('#secure-storage-modal .modal-message').html("Storing Recipient's Bank Account on Device...");$('#secure-storage-modal').get(0).show();// show loader
// update the bank account image/avatar property
utopiasoftware.saveup.controller.addRecipientPageViewModel.bankAccountImage="css/app-images/avatar-"+utopiasoftware.saveup.controller.addRecipientPageViewModel.bankAccountImageRandomNum+".png";// check if this is an EDIT or CREATE operation
if($('#app-main-navigator').get(0).topPage.data&&$('#app-main-navigator').get(0).topPage.data.edit){//this is an EDIT operation
// call the utility method used to delete a specified bank account
utopiasoftware.saveup.savedRecipientsBankAccountOperations.deleteSavedRecipientAccount($('#app-main-navigator').get(0).topPage.data.edit).then(function(){// create an edited bank account data
var editedBankAcctData={uniqueAccountId:$('#app-main-navigator').get(0).topPage.data.edit,bankAccountNumber:$('#add-recipient-form #add-recipient-account-number').val(),bankAccountName:$('#add-recipient-form #add-recipient-account-name').val(),bankName:$('#add-recipient-form #add-recipient-choose-bank option:selected').text().trim(),flutterwave_bankCode:$('#add-recipient-form #add-recipient-choose-bank').val(),bankAccountAvatar:utopiasoftware.saveup.controller.addRecipientPageViewModel.bankAccountImage};// call the utility method used to add a specified bank account to the bank account collection
return utopiasoftware.saveup.savedRecipientsBankAccountOperations.addSavedRecipientAccount(editedBankAcctData);}).then(function(){// wait for approximately 4 secs for the saving animation to run (at least once before concluding animation
window.setTimeout(function(){// reset the form validator object on the page
utopiasoftware.saveup.controller.addRecipientPageViewModel.formValidator.reset();// reset the form object
$('#add-recipient-page #add-recipient-form').get(0).reset();$('#add-recipient-page select').material_select();// reset the account number validation check and all its related actions
...
if($(checkElem).is(":checked")){// user wants the account number remotely validated
// add the necessary attributes to the account number input in order to enable remote account number validation
$('#add-recipient-account-number').attr("data-parsley-remote-validator","recipientaccountnumbercheck");// also make the account name input disabled & wipe its previous content
$('#add-recipient-account-name').attr("disabled",true);$('#add-recipient-account-name').val("");// remove tooltip from account name
$("#add-recipient-account-name").parent().find('label:eq(0)').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$("#add-recipient-account-name").parent().find('label:eq(0)').removeAttr("data-hint");// display the notice that an internet connection is required for verification
$('#add-recipient-account-verify-account-notice').css("display","block");}else{// user does not want account number remotely validated
// remove attributes to prevent remote account number validation
$('#add-recipient-account-number').removeAttr("data-parsley-remote-validator");// also make the account name input enabled & wipe its previous content
$('#add-recipient-account-name').removeAttr("disabled");$('#add-recipient-account-name').val("");// hide the notice that an internet connection is required for verification
$('#add-recipient-account-verify-account-notice').css("display","none");}},/**
         * method is used to listen for scroll event of the page content
         *
         * @param event
         */pageContentScrolled:function pageContentScrolled(event){// set the current scrolltop position
utopiasoftware.saveup.controller.addRecipientPageViewModel.currentScrollPosition=$(this).scrollTop();if(utopiasoftware.saveup.controller.addRecipientPageViewModel.currentScrollPosition>utopiasoftware.saveup.controller.addRecipientPageViewModel.previousScrollPosition){// user scrolled up
// set the current position as previous position
utopiasoftware.saveup.controller.addRecipientPageViewModel.previousScrollPosition=utopiasoftware.saveup.controller.addRecipientPageViewModel.currentScrollPosition;// check if the header image left after scrolling is <= height of page toolbar
if(140-utopiasoftware.saveup.controller.addRecipientPageViewModel.currentScrollPosition<=56){// the header image left after scrolling is <= height of page toolbar
// check if the toolbar for the page has already been made opaque
if(this.isToolBarOpaque!=true){// toolbar has not been made opaque
$('#add-recipient-page ons-toolbar').removeClass("toolbar--transparent translucent-toolbar");// make the toolbar opaque
// also pin the help header on the page just below the toolbar
$('#add-recipient-page ons-list-header').css({"display":"block","position":"fixed","top":"56px","width":"100%"});this.isToolBarOpaque=true;// flag that toolbar has been made opaque
}}return;}if(utopiasoftware.saveup.controller.addRecipientPageViewModel.currentScrollPosition<utopiasoftware.saveup.controller.addRecipientPageViewModel.previousScrollPosition){// user scrolled down
// set the current position as previous position
utopiasoftware.saveup.controller.addRecipientPageViewModel.previousScrollPosition=utopiasoftware.saveup.controller.addRecipientPageViewModel.currentScrollPosition;// check if the header image left after scrolling is > height of page toolbar
if(140-utopiasoftware.saveup.controller.addRecipientPageViewModel.currentScrollPosition>56){// the header image left after scrolling is > height of page toolbar
// check if the toolbar for the page has already been made transparent
...
// also unpin the help header on the page from just below the toolbar
$('#add-recipient-page ons-list-header').css({"display":"block","position":"static","top":"56px"});this.isToolBarOpaque=false;// flag that toolbar has been made transparent
}}return;}},/**
         * method is used to reset/wipe the content of the account name IF
         * remote account validation has been enabled
         */resetAccountNameDisplay:function resetAccountNameDisplay(){if($('#add-recipient-page #add-recipient-account-verify-account').is(":checked")){// user wants the account number remotely validated
$('#add-recipient-page #add-recipient-account-name').val("");}},/**
         * custom parsley validator for bank account number
         *
         * @param jqxhr {jqueryXhr}
         */recipientAccountNumberValidator:function recipientAccountNumberValidator(jqxhr){var serverResponse="";// holds the server response
// check the validator response
if(jqxhr.status!=200){// request was NOT success
return false;}// convert the server response to json
serverResponse=JSON.parse(jqxhr.responseText.trim());if(serverResponse.status!="success"){// the server api response was NOT successful
return false;}else{// the server api response was successful
// set the bank account name
utopiasoftware.saveup.controller.addRecipientPageViewModel.bankAccountName=serverResponse.data.account_name;return true;// validation successful
}}},/**
...
// find all onsen-ui input targets and insert a special class to prevent materialize-css from updating the styles
$('ons-input input',$thisPage).addClass('utopiasoftware-no-style');// enable the swipeable feature for the app splitter
$('ons-splitter-side').attr("swipeable",true);// reset the previous & current scroll positions of the page contents
utopiasoftware.saveup.controller.transferCashCardPageViewModel.previousScrollPosition=0;utopiasoftware.saveup.controller.transferCashCardPageViewModel.currentScrollPosition=0;// call the function used to initialise the app page if the app is fully loaded
loadPageOnAppReady();//function is used to initialise the page if the app is fully ready for execution
function loadPageOnAppReady(){// check to see if onsen is ready and if all app loading has been completed
if(!ons.isReady()||utopiasoftware.saveup.model.isAppReady===false){setTimeout(loadPageOnAppReady,500);// call this function again after half a second
return;}// listen for the back button event
...
$('#app-main-navigator').get(0).resetToPage("main-menu-page.html");}else{// the second tab is active
// move the tab view to the Card tab
$('.transfer-cash-tabbar').get(0).setActiveTab(0,{animation:"slide"});}};// listen for the scroll event of the page carousel content
$('ons-carousel',$thisPage).on("scroll",utopiasoftware.saveup.controller.transferCashCardPageViewModel.pageContentScrolled);// initialise the form validation objects.
utopiasoftware.saveup.controller.transferCashCardPageViewModel.transferAmountFieldValidator=$('#transfer-cash-card-amount',$thisPage).parsley({// used solely for the transfer-cash-amount input
value:function value(parsley){// convert the amount back to a plain text without the thousand separator
var parsedNumber=kendo.parseFloat($('#transfer-cash-card-amount',$thisPage).val());return parsedNumber?parsedNumber:$('#transfer-cash-card-amount',$thisPage).val();}});utopiasoftware.saveup.controller.transferCashCardPageViewModel.formValidator=$('#transfer-cash-card-form',$thisPage).parsley();// used for the form in general
// attach listener for the 'transfer cash' card button click
$('#transfer-cash-card-button',$thisPage).get(0).onclick=function(){// run the validation method for the transfer-cash-card form
...
$('#transfer-cash-card-number.autocomplete',$thisPage).autocomplete({data:autoCompleteData,onAutocomplete:function onAutocomplete(val){// callback function when value is autocompleted
...
$('#transfer-cash-card-recipient-account-name.autocomplete',$thisPage).autocomplete({data:autoCompleteData,onAutocomplete:function onAutocomplete(val){// Callback function when value is autcompleted.
},minLength:1// The minimum length of the input for the autocomplete to start. Default: 1.
...
// initialise all the select element
$('select',$thisPage).material_select();return null;},function(){return null;}).then(function(){var pageDataObject=$('#app-main-navigator').get(0).topPage.data;// get the data attached to this page
...
$('.transfer-cash-tabbar').get(0).setActiveTab(1,{animation:"slide"});}// check if the data contains sender card details
...
cashTransferRequest.fail(function(jqxhr){if(jqxhr.status==500){reject(JSON.parse(jqxhr.responseText.trim()));}else{reject(jqxhr.responseText.trim());}});});}}).then(function(responseData){// securely store the transaction history from the responseData on the user's device
responseData.data.transfer.postcash_transaction_type="Cash Transfer (Card)";// set the postcash transaction type
return Promise.all([utopiasoftware.saveup.transactionHistoryOperations.addTransactionHistory(responseData.data.transfer),Promise.resolve(responseData)]);}).then(function(responseDataArray){if(responseDataArray==null||responseDataArray[1]==null){// user already terminated transfer
...
$('.postcash-preloader-transfer-cash-card-form-container',utopiasoftware.saveup.controller.transferCashCardPageViewModel.formValidator.$element).css("display","block");// split the input value so can can extract just the card number
...
$('.postcash-transfer-cash-card-hidden-info:eq(0)',utopiasoftware.saveup.controller.transferCashCardPageViewModel.formValidator.$element).css("display","none");}if((cardObject.cardBrand=="Mastercard"||cardObject.cardBrand=="Verve")&&cardObject.cardLocale=="local"){// card is a local mastercard or verve
...
Materialize.updateTextFields();},/**
         ...
         * @param event
         */pageContentScrolled:function pageContentScrolled(event){// set the current scrolltop position
utopiasoftware.saveup.controller.transferCashCardPageViewModel.currentScrollPosition=$(this).scrollTop();if(utopiasoftware.saveup.controller.transferCashCardPageViewModel.currentScrollPosition>utopiasoftware.saveup.controller.transferCashCardPageViewModel.previousScrollPosition){// user scrolled up
// set the current position as previous position
utopiasoftware.saveup.controller.transferCashCardPageViewModel.previousScrollPosition=utopiasoftware.saveup.controller.transferCashCardPageViewModel.currentScrollPosition;// check if the user has scrolled at least 10px up
if(utopiasoftware.saveup.controller.transferCashCardPageViewModel.currentScrollPosition>=10){// the user scrolled at least 10px up
...
}}return;}if(utopiasoftware.saveup.controller.transferCashCardPageViewModel.currentScrollPosition<utopiasoftware.saveup.controller.transferCashCardPageViewModel.previousScrollPosition){// user scrolled down
// set the current position as previous position
...
$('ons-input input',$thisPage).addClass('utopiasoftware-no-style');// enable the swipeable feature for the app splitter
$('ons-splitter-side').attr("swipeable",true);// reset the previous & current scroll positions of the page contents
utopiasoftware.saveup.controller.transferCashBankPageViewModel.previousScrollPosition=0;utopiasoftware.saveup.controller.transferCashBankPageViewModel.currentScrollPosition=0;// call the function used to initialise the app page if the app is fully loaded
loadPageOnAppReady();//function is used to initialise the page if the app is fully ready for execution
function loadPageOnAppReady(){// check to see if onsen is ready and if all app loading has been completed
if(!ons.isReady()||utopiasoftware.saveup.model.isAppReady===false){setTimeout(loadPageOnAppReady,500);// call this function again after half a second
return;}// listen for the back button event
...
// move the tab view to the Card tab
$('.transfer-cash-tabbar').get(0).setActiveTab(0,{animation:"slide"});}};// listen for the scroll event of the page carousel content
$('ons-carousel',$thisPage).on("scroll",utopiasoftware.saveup.controller.transferCashBankPageViewModel.pageContentScrolled);// initialise the form validation objects.
utopiasoftware.saveup.controller.transferCashBankPageViewModel.transferAmountFieldValidator=$('#transfer-cash-bank-amount',$thisPage).parsley({// used solely for the transfer-cash-amount input
value:function value(parsley){// convert the amount back to a plain text without the thousand separator
var parsedNumber=kendo.parseFloat($('#transfer-cash-bank-amount',$thisPage).val());return parsedNumber?parsedNumber:$('#transfer-cash-bank-amount',$thisPage).val();}});utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator=$('#transfer-cash-bank-form',$thisPage).parsley();// used for the form in general
// attach listener for the 'transfer cash' card button click
$('#transfer-cash-bank-button',$thisPage).get(0).onclick=function(){// run the validation method for the transfer-cash-bank form
utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.whenValidate();};// listen for form field validation failure event
utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.on('field:error',function(fieldInstance){// get the element that triggered the field validation error and use it to display tooltip
// display tooltip
$(fieldInstance.$element).parent().find('label:eq(0)').addClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).parent().find('label:eq(0)').attr("data-hint",fieldInstance.getErrorsMessages()[0]);});// listen for form field validation success event
utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.on('field:success',function(fieldInstance){// remove tooltip from element
$(fieldInstance.$element).parent().find('label:eq(0)').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).parent().find('label:eq(0)').removeAttr("data-hint");});// listen for transfer-cash-Bank form validation success
utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.on('form:success',utopiasoftware.saveup.controller.transferCashBankPageViewModel.transferCashBankFormValidated);// initialise the sender account autocomplete widget
utopiasoftware.saveup.bankAccountOperations.loadMyAccountsData().then(function(bankAcctArrayData){var autoCompleteData={};// holds the data used to initialise the autocomplete widget
bankAcctArrayData.forEach(function(arrayElem){// function to convert each array element to a format for autocomplete
autoCompleteData[arrayElem.bankAccountName+" - "+arrayElem.bankAccountNumber]=arrayElem.bankAccountAvatar;});// intialise widget
$('#transfer-cash-bank-sender-account-name.autocomplete',$thisPage).autocomplete({data:autoCompleteData,onAutocomplete:function onAutocomplete(val){// callback function when value is autocompleted
},minLength:1// The minimum length of the input for the autocomplete to start. Default: 1.
});// call the method to retrieve saved bank account recipient's used to populate recipient autocomplete widget
return utopiasoftware.saveup.savedRecipientsBankAccountOperations.loadSavedRecipientsAccountsData();},function(){return[];}).then(function(savedRecipientsArray){var autoCompleteData={};// holds the data used to initialise the autocomplete widget
savedRecipientsArray.forEach(function(arrayElem){// function to convert each array element to a format for autocomplete
autoCompleteData[arrayElem.bankAccountName+" - "+arrayElem.bankAccountNumber]=arrayElem.bankAccountAvatar;});// initialise widget
$('#transfer-cash-bank-recipient-account-name.autocomplete',$thisPage).autocomplete({data:autoCompleteData,onAutocomplete:function onAutocomplete(val){// Callback function when value is autcompleted.
},minLength:1// The minimum length of the input for the autocomplete to start. Default: 1.
});return Promise.all([utopiasoftware.saveup.filteredSenderBanksData(),utopiasoftware.saveup.sortBanksData()]);// retrieve data for banks
},function(){return[[],[]];}).then(function(bankArrayData){var optionTags="";// string to hold all created option tags
// get each object in bank array and use it to create the 'bank' select element
for(var index=0;index<bankArrayData[0].length;index++){var bankObject=bankArrayData[0][index];// get the bank object
// update the banks select element option tags
for(var prop in bankObject){optionTags+='<option value="'+prop+'">'+bankObject[prop]+'</option>';}}$('#transfer-cash-bank-sender-choose-bank',$thisPage).append(optionTags);// append all the created option tags
// initialise the 'sender bank' select element
$('#transfer-cash-bank-sender-choose-bank',$thisPage).material_select();return bankArrayData;// return at this point, so the page can continue initialisation
},function(){return[[],[]];}).then(function(bankArrayData){var optionTags="";// string to hold all created option tags
// get each object in bank array and use it to create the 'bank' select element
for(var index=0;index<bankArrayData[1].length;index++){var bankObject=bankArrayData[1][index];// get the bank object
// update the banks select element option tags
for(var prop in bankObject){optionTags+='<option value="'+prop+'">'+bankObject[prop]+'</option>';}}$('#transfer-cash-bank-recipient-choose-bank',$thisPage).append(optionTags);// append all the created option tags
// initialise the 'recipient bank' select element
$('#transfer-cash-bank-recipient-choose-bank',$thisPage).material_select();return bankArrayData;// return at this point, so the page can continue initialisation
},function(){return null;}).then(function(){var pageDataObject=$('#app-main-navigator').get(0).topPage.data;// get the data attached to this page
// check if the data contains recipient account details
if(pageDataObject&&pageDataObject.recipient_details){// recipient account details are present
// update the form details to contains the received recipient account details
...
$('ons-splitter-side').attr("swipeable",true);},/**
         ...
         */pageHide:function pageHide(event){var $thisPage=$(event.target);// get the current page shown
try{// remove any tooltip being displayed on all forms on the page
$('[data-hint]',$thisPage).removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$('[data-hint]',$thisPage).removeAttr("data-hint");// reset the transfer-cash-bank form validator object on the page
utopiasoftware.saveup.controller.transferCashBankPageViewModel.transferAmountFieldValidator.reset();utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.reset();}catch(err){}},/**
         * method is triggered when the page is destroyed
         * @param event
         */pageDestroy:function pageDestroy(event){var $thisPage=$(event.target);// get the current page shown
try{// remove any tooltip being displayed on all forms on the page
$('[data-hint]',$thisPage).removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$('[data-hint]',$thisPage).removeAttr("data-hint");// destroy the form validator objects on the page
utopiasoftware.saveup.controller.transferCashBankPageViewModel.transferAmountFieldValidator.reset();utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.destroy();// destroy the form inputs which need to be destroyed
$('select',$thisPage).material_select('destroy');$('input.autocomplete',$thisPage).off();$('input.autocomplete',$thisPage).removeData();}catch(err){}},/**
         * method is triggered when transfer-cash-bank form is successfully validated
         */transferCashBankFormValidated:function transferCashBankFormValidated(){// check if Internet Connection is available before proceeding
if(navigator.connection.type===Connection.NONE){// no Internet Connection
// inform the user that they cannot proceed without Internet
window.plugins.toast.showWithOptions({message:"You cannot transfer cash without an active Internet Connection",duration:4500,position:"top",styling:{opacity:1,backgroundColor:'#ff0000',//red
textColor:'#FFFFFF',textSize:14}},function(toastEvent){if(toastEvent&&toastEvent.event=="touch"){// user tapped the toast, so hide toast immediately
...
cashTransferRequest.fail(function(jqxhr){if(jqxhr.status==500){reject(JSON.parse(jqxhr.responseText.trim()));}else{reject(jqxhr.responseText.trim());}});});}}).then(function(responseData){// securely store the transaction history from the responseData on the user's device
var transferRecipientDetailsArray=$('#transfer-cash-bank-recipient-account-name','#transfer-cash-bank-page').val().split(" - ");// holds the details of the transfer recipient in an array gotten from the 'transfer-cash-bank-recipient-account-name' input
...
responseData.data.transfer.postcash_transferDetails={narration:$('#transfer-cash-bank-narration','#transfer-cash-bank-page').val(),sender_bank:$('#transfer-cash-bank-sender-choose-bank','#transfer-cash-bank-page').val(),sender_account_number:$('#transfer-cash-bank-sender-account-name','#transfer-cash-bank-page').val().split(" - ").pop(),recipient_bank:$('#transfer-cash-bank-recipient-choose-bank','#transfer-cash-bank-page').val(),recipient_account_name:transferRecipientDetailsArray.length>1?transferRecipientDetailsArray[transferRecipientDetailsArray.length-2]:"",recipient_account_number:transferRecipientDetailsArray.pop()};return Promise.all([utopiasoftware.saveup.transactionHistoryOperations.addTransactionHistory(responseData.data.transfer),Promise.resolve(responseData)]);}).then(function(responseDataArray){if(responseDataArray==null||responseDataArray[1]==null){// user already terminated transfer
throw null;}var responseData=responseDataArray[1];// retrieve the response from the server
...
/*
                    ...
                    */// update the transaction indicators
$('.postcash-pay-progress .col:eq(0) span:eq(0)').removeClass('postcash-pay-progress-milestone-active').addClass('postcash-pay-progress-milestone');$('.postcash-pay-progress .col:eq(0) span:eq(1)').removeClass('postcash-pay-progress-milestone-text-active').addClass('postcash-pay-progress-milestone-text');$('.postcash-pay-progress .col:eq(1) span:eq(0)').removeClass('postcash-pay-progress-milestone').addClass('postcash-pay-progress-milestone-active');$('.postcash-pay-progress .col:eq(1) span:eq(1)').removeClass('postcash-pay-progress-milestone-text').addClass('postcash-pay-progress-milestone-text-active');// transition to 'authorize' block
return $('.transfer-cash-bank-carousel').get(0).next({animation:'none'});}).then(function(object){if(object==null){// user already terminated transfer
throw null;}$('#loader-modal').get(0).hide();// hide loader
}).catch(function(error){$('#loader-modal').get(0).hide();// hide loader
if(error==null){// the user decided NOT to proceed with the transfer
return;// do nothing
         ...
         *
         */transferCashBankRemoteAuthorize:function transferCashBankRemoteAuthorize(authorizationStage,authorizationResponseData){var secureToken=null;// holds the secure transaction token
...
cashTransferAuthorisation.fail(function(jqxhr){reject(JSON.parse(jqxhr.responseText.trim()));});});}).then(function(responseData){// send the server data to the postcash business logic container
var transferRecipientDetailsArray=$('#transfer-cash-bank-recipient-account-name','#transfer-cash-bank-page').val().split(" - ");// holds the details of the transfer recipient in an array gotten from the 'transfer-cash-bank-recipient-account-name' input
// create the data to be sent to the postcash business logic container
var transferData={transferData:responseData.data,authorizationToken:secureToken,postcash_fee:utopiasoftware.saveup.model.fee,postcash_transferDetails:{narration:$('#transfer-cash-bank-narration','#transfer-cash-bank-page').val(),sender_bank:$('#transfer-cash-bank-sender-choose-bank','#transfer-cash-bank-page').val(),sender_account_number:$('#transfer-cash-bank-sender-account-name','#transfer-cash-bank-page').val().split(" - ").pop(),recipient_bank:$('#transfer-cash-bank-recipient-choose-bank','#transfer-cash-bank-page').val(),recipient_account_name:transferRecipientDetailsArray.length>1?transferRecipientDetailsArray[transferRecipientDetailsArray.length-2]:"",recipient_account_number:transferRecipientDetailsArray.pop()}};// update the transaction history with the 'postcash_transferDetails' property and move the transferData object to the next phase
return Promise.all([utopiasoftware.saveup.transactionHistoryOperations.updateTransactionHistoryData(window.sessionStorage.getItem("transaction_ref"),{postcash_transferDetails:transferData.postcash_transferDetails}),Promise.resolve(transferData)]);}).then(function(transferDataArray){// post the transfer data to the transfer authorization iframe for stage 2 processing
$('#transfer-cash-bank-authorise-modal #transfer-cash-bank-authorise-iframe').get(0).contentWindow.postMessage(JSON.stringify(transferDataArray[1]),"https://postcash.000webhostapp.com");}).catch(function(error){// update the transaction history for the cash transfer (bank) transaction to mark failure
...
$('#transfer-cash-bank-authorise-iframe').attr("src","");$('#transfer-cash-bank-authorise-iframe').attr("src",window.sessionStorage.getItem("transaction_authurl"));// show the modal that contains the modal
$('#transfer-cash-bank-authorise-modal').get(0).show();},/**
         * method is used to listen for scroll event of the page content
         *
         * @param event
         */pageContentScrolled:function pageContentScrolled(event){// set the current scrolltop position
utopiasoftware.saveup.controller.transferCashBankPageViewModel.currentScrollPosition=$(this).scrollTop();if(utopiasoftware.saveup.controller.transferCashBankPageViewModel.currentScrollPosition>utopiasoftware.saveup.controller.transferCashBankPageViewModel.previousScrollPosition){// user scrolled up
// set the current position as previous position
utopiasoftware.saveup.controller.transferCashBankPageViewModel.previousScrollPosition=utopiasoftware.saveup.controller.transferCashBankPageViewModel.currentScrollPosition;// check if the user has scrolled at least 10px up
if(utopiasoftware.saveup.controller.transferCashBankPageViewModel.currentScrollPosition>=10){// the user scrolled at least 10px up
// check if the tab-bar is visible
if($('.transfer-cash-tabbar').last().get(0).visible==true){// tab-bar is visible
$('.transfer-cash-tabbar').last().get(0).setTabbarVisibility(false);// hide the tab-bar
}}return;}if(utopiasoftware.saveup.controller.transferCashBankPageViewModel.currentScrollPosition<utopiasoftware.saveup.controller.transferCashBankPageViewModel.previousScrollPosition){// user scrolled down
// set the current position as previous position
utopiasoftware.saveup.controller.transferCashBankPageViewModel.previousScrollPosition=utopiasoftware.saveup.controller.transferCashBankPageViewModel.currentScrollPosition;// check if the tabbar is hidden
if($('.transfer-cash-tabbar').last().get(0).visible==false){// tab-bar is hidden
$('.transfer-cash-tabbar').last().get(0).setTabbarVisibility(true);// show the tab-bar
}return;}},/**
         ...
         *
         * @param inputElem
         */senderAccountNumberChanged:function senderAccountNumberChanged(inputElem){// display the preloaders that block input so certain account information can be autofilled
$('.postcash-preloader-transfer-cash-bank-form-container',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).css("display","block");// split the input value so can can extract just the recipient account number
var valueSplitArray=$(inputElem).val().split(" - ");//get the account number from the array
var accountNumber=valueSplitArray.pop();// get the user account object from secure storage using the retrieved account number
utopiasoftware.saveup.bankAccountOperations.getMyAccountByNumber(accountNumber).then(function(acctObject){// autofill the contents of the cash transfer form (sender section) with the contents of sender account object
if(!acctObject){// account object is null
throw"error";}$('#transfer-cash-bank-sender-choose-bank',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).val(acctObject.flutterwave_bankCode);$('#hidden-choose-sender-bank-input',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).val(acctObject.flutterwave_bankCode);// remove any tooltip being displayed on the transfer cash form
$('[data-hint]',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$('[data-hint]',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).removeAttr("data-hint");// update the display of the updated fields
//Materialize.updateTextFields();
$('select',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).material_select();// hide the preloaders that block input, autofill has been completed
$('.postcash-preloader-transfer-cash-bank-form-container',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).css("display","none");}).catch(function(){// an error occurred OR user entered a recipient account that has not been previously saved
// update the display of the updated fields
//Materialize.updateTextFields();
...
$('.postcash-preloader-transfer-cash-bank-form-container',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).css("display","none");});},/**
         ...
         *
         * @param inputElem
         */recipientAccountNumberChanged:function recipientAccountNumberChanged(inputElem){// display the preloaders that block input so certain account information can be autofilled
$('.postcash-preloader-transfer-cash-bank-form-container',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).css("display","block");// split the input value so can can extract just the recipient account number
var valueSplitArray=$(inputElem).val().split(" - ");//get the account number from the array
var accountNumber=valueSplitArray.pop();// get the recipient account object from secure storage using the retrieved account number
utopiasoftware.saveup.savedRecipientsBankAccountOperations.getSavedRecipientAccountByNumber(accountNumber).then(function(acctObject){// autofill the contents of the cash transfer form (recipient section) with the contents of recipient account object
if(!acctObject){// account object is null
throw"error";}$('#transfer-cash-bank-recipient-choose-bank',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).val(acctObject.flutterwave_bankCode);$('#hidden-choose-recipient-bank-input',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).val(acctObject.flutterwave_bankCode);// remove any tooltip being displayed on the transfer cash form
$('[data-hint]',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$('[data-hint]',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).removeAttr("data-hint");// update the display of the updated fields
//Materialize.updateTextFields();
$('select',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).material_select();// hide the preloaders that block input, autofill has been completed
$('.postcash-preloader-transfer-cash-bank-form-container',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).css("display","none");}).catch(function(){// an error occurred OR user entered a recipient account that has not been previously saved
...
//Materialize.updateTextFields();
$('select',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).material_select();// hide the preloaders that block input
$('.postcash-preloader-transfer-cash-bank-form-container',utopiasoftware.saveup.controller.transferCashBankPageViewModel.formValidator.$element).css("display","none");});}}},_defineProperty(_utopiasoftware$saveu,'onboardingPageViewModel',{/**
...
//set the first page to be displayed to be the login page
$('ons-splitter').get(0).content.load("login-template");}}}),_defineProperty(_utopiasoftware$saveu,'transactionHistoryPageViewModel',{/**
         * event is triggered when page is initialised
         */pageInit:function pageInit(event){var $thisPage=$(event.target);// get the current page shown
// enable the swipeable feature for the app splitter
$('ons-splitter-side').attr("swipeable",true);// call the function used to initialise the app page if the app is fully loaded
...
$('#transaction-history-pull-hook',$thisPage).on("changestate",function(event){// check the state of the pull-to-refresh widget
switch(event.originalEvent.state){case'initial':// update the displayed icon
$('#transaction-history-pull-hook-fab',event.originalEvent.pullHook).html('<ons-icon icon="fa-long-arrow-down" size="24px"></ons-icon>');break;case'preaction':$('#transaction-history-pull-hook-fab',event.originalEvent.pullHook).html('<ons-icon icon="fa-long-arrow-up" size="24px"></ons-icon>');break;case'action':$('#transaction-history-pull-hook-fab',event.originalEvent.pullHook).html('<ons-icon icon="fa-repeat" size="24px" spin></ons-icon>');break;}});// add method to handle the loading action of the pull-to-refresh widget
$('#transaction-history-pull-hook',$thisPage).get(0).onAction=function(loadingDone){// disable pull-to-refresh widget till loading is done
$('#transaction-history-pull-hook',$thisPage).attr("disabled",true);// load the user's transaction history data from the device secure store
utopiasoftware.saveup.transactionHistoryOperations.loadTransactionHistoryData().then(function(transactionHistoryArray){// the user's transaction history data array collection has been returned
...
transactionHistoryContent+='<span class="transaction-history-item-success-indicator">\n                                            </span><span class="transaction-history-item-time-indicator">\n                                            '+kendo.toString(new Date(transactionHistoryArray[index].createdAt),"HH:mm yyyy-MM-dd")+'</span></ons-col>\n                                                <ons-col width="75%" class="transaction-history-content-container">\n                                            <div style="font-size: 0.8em; width: 100%">\n                                            <span class="transaction-history-content-label">\n                                            Transaction Type\n                                            </span>\n                                            <span>\n                                            '+transactionHistoryArray[index].postcash_transaction_type+'\n                                            </span></div>\n                                            <div style="font-size: 0.8em; width: 100%">\n                                            <span class="transaction-history-content-label">\n                                            Transaction Amount (NGN)\n                                            </span><span>\n                                            '+kendo.toString(kendo.parseFloat(transactionHistoryArray[index].amountToCharge),"n2")+'\n                                            </span></div>\n                                            <div style="font-size: 0.8em; width: 100%">\n                                            <span class="transaction-history-content-label">\n                                            Transaction Status\n                                            </span><span>Successful</span></div>\n                                            <div style="font-size: 0.8em; width: 100%">\n                                            <span  class="transaction-history-content-label">\n                                            Beneficiary\n                                            </span><span>\n                                            '+(transactionHistoryArray[index].postcash_transferDetails.recipient_account_number+" "+transactionHistoryArray[index].postcash_transferDetails.recipient_account_name)+'\n                                            </span></div>\n                                            <div style="font-size: 0.8em; width: 100%">\n                                            <ons-button class="transaction-history-content-button"\n                                                data-transaction-id="'+transactionHistoryArray[index].flutterChargeReference+'"\n                                                onclick="utopiasoftware.saveup.controller.transactionHistoryPageViewModel.saveDetailsButtonClicked(this)">\n                                            Save Details\n                                            </ons-button>\n                                            <ons-button class="transaction-history-content-button"\n                                            data-transaction-id="'+transactionHistoryArray[index].flutterChargeReference+'"\n                                                onclick="utopiasoftware.saveup.controller.transactionHistoryPageViewModel.sendFeedbackButtonClicked(this)">\n                                            Send Feedback\n                                            </ons-button>\n                                            </div></ons-col>';}break;// end of "Cash Transfer (Bank)" transaction type
}// append the transaction history content to the "Transaction History" list
$('#transaction-history-list .transaction-history-items-container',$thisPage).append(transactionHistoryContent);transactionHistoryContent='';// reset the content in prepareation for another loop
}// remove the page preloader progress bar
$('.progress',$thisPage).remove();// display the help button
$('#transaction-history-help-1',$thisPage).css("display","inline-block");// enable the pull-to-refresh widget for the page
...
$('#transaction-history-list',$thisPage).css("display","none");// flag that loading is done
loadingDone();});};// load the user's transaction history data from the device secure store
utopiasoftware.saveup.transactionHistoryOperations.loadTransactionHistoryData().then(function(transactionHistoryArray){// the user's transaction history data array collection has been returned
if(transactionHistoryArray.length==0){// there are no transaction history data available
// remove the page preloader progress bar
$('.progress',$thisPage).remove();// display the help button
...
$('#transaction-history-page-message',$thisPage).css("display","block");// hide the error message from displaying
$('#transaction-history-page-error',$thisPage).css("display","none");// hide the transaction-history-list from display
$('#transaction-history-list',$thisPage).css("display","none");}else{// there are transaction history data available
// empty the contents of the transaction history list
$('#transaction-history-list .transaction-history-items-container',$thisPage).html("");// create the transaction history content
var transactionHistoryContent='';for(var index=0;index<transactionHistoryArray.length;index++){// append the stored transaction history to the "Transaction History" list
// check what type of postcash transaction it is
...
$('#transaction-history-page-message',$thisPage).css("display","none");// hide the error message from displaying
$('#transaction-history-page-error',$thisPage).css("display","none");// display the transaction-history-list
$('#transaction-history-list',$thisPage).css("display","block");}}).catch(function(){// an error occurred, so display the error message to the user
// remove the page preloader progress bar
$('.progress',$thisPage).remove();// display the help button
$('#transaction-history-help-1',$thisPage).css("display","inline-block");// enable the pull-to-refresh widget for the page
$('#transaction-history-pull-hook',$thisPage).removeAttr("disabled");// hide a message to inform user that there are no transaction history data available
$('#transaction-history-page-message',$thisPage).css("display","none");// display the error message to user
$('#transaction-history-page-error',$thisPage).css("display","block");// hide the transaction-history-list from display
$('#transaction-history-list',$thisPage).css("display","none");});// hide the loader
$('#loader-modal').get(0).hide();}},/**
         * method is triggered when page is shown
         *
         * @param event
...
$('#app-main-navigator').get(0).topPage.alwaysShowSecurityLockModal=true;},/**
         ...
         *
         * @param buttonElem
         */saveDetailsButtonClicked:function saveDetailsButtonClicked(buttonElem){var fileContentBlob=null;// holds the blob content of the file to be stored on user's device
...
$('#loader-modal-message').html("Saving Transaction Details...");$('#loader-modal').get(0).show().then(function(){// get the specific transaction history which is to be saved
var transactionRef=$(buttonElem).attr("data-transaction-id");return utopiasoftware.saveup.transactionHistoryOperations.getTransactionHistoryById(transactionRef);}).then(function(transactionData){// receive the requested transaction data
if(!transactionData){// no transaction data was found
throw{"message":"Transaction not found."};}return transactionData;}).then(function(transactionData){// use the receive transaction data to create the necessary document/file DOM
// empty the contents of the transaction-history-saver-container
$('#transaction-history-saver-container').html("");var transactionHistoryDocContent='<h3 style="text-align: center; min-width: 100%; width: 100%; margin-top: 1.5em;">\n                PostCash Transaction Details</h3>\n                <div style="text-align: right; min-width: 90%; max-width: 90%; width: 90%;">\n                <span style="font-weight: bold; font-size: 1.5em">Transaction Date: </span>\n                <span style="font-size: 1.5em">'+kendo.toString(new Date(transactionData.createdAt),"HH:mm yyyy-MM-dd")+'</span>\n                </div>\n                <table style="margin-top: 1.5em; border: 2px #000000 solid; min-width: 100%; width: 100%;">\n\n                <tr>\n                <td style="min-width:30%; width:30%; font-size: 1.5em; font-weight: bold; border: 1px #000000 solid;">\n                Transaction Reference\n                </td>\n                <td style="min-width:70%; width:70%; font-size: 1.5em; border: 1px #000000 solid;">\n                '+transactionData.flutterChargeReference+'\n                </td>\n                </tr>';// handle the content to be created based on the postcash transaction type
switch(transactionData.postcash_transaction_type){case"Cash Transfer (Card)":// transaction type is "Cash Transfer (Card)"
transactionHistoryDocContent+='<tr>\n                        <td style="min-width:30%; width:30%; font-size: 1.5em; font-weight: bold; border: 1px #000000 solid;">';if(transactionData.flutterChargeResponseMessage.toLocaleUpperCase()!="APPROVED"){// transaction is pending
transactionHistoryDocContent+='Transaction Type </td>\n                                            <td style="min-width:70%; width:70%; font-size: 1.5em; border: 1px #000000 solid;">\n                                            '+transactionData.postcash_transaction_type+'\n                                            </td></tr>\n                                            <tr>\n                        <td style="min-width:30%; width:30%; font-size: 1.5em; font-weight: bold; border: 1px #000000 solid;">\n                                            Transaction Amount (NGN) </td>\n                                            <td style="min-width:70%; width:70%; font-size: 1.5em; border: 1px #000000 solid;">\n                                            '+kendo.toString(kendo.parseFloat(transactionData.amountToCharge),"n2")+'\n                                            </td></tr>\n                                            <tr>\n                        <td style="min-width:30%; width:30%; font-size: 1.5em; font-weight: bold; border: 1px #000000 solid;">\n                                            Transaction Status</td>\n                                            <td style="min-width:70%; width:70%; font-size: 1.5em; border: 1px #000000 solid;">\n                                            Incomplete\n                                            </td></tr>\n                                            <tr>\n                        <td style="min-width:30%; width:30%; font-size: 1.5em; font-weight: bold; border: 1px #000000 solid;">\n                                            Beneficiary </td>\n                                            <td style="min-width:70%; width:70%; font-size: 1.5em; border: 1px #000000 solid;">\n                                            '+(transactionData.beneficiary.accountNumber+" - "+transactionData.beneficiary.accountName)+'\n                                            </td></tr></table>';}else{// transaction is successful
transactionHistoryDocContent+='Transaction Type </td>\n                                            <td style="min-width:70%; width:70%; font-size: 1.5em; border: 1px #000000 solid;">\n                                            '+transactionData.postcash_transaction_type+'\n                                            </td></tr>\n                                            <tr>\n                        <td style="min-width:30%; width:30%; font-size: 1.5em; font-weight: bold; border: 1px #000000 solid;">\n                                            Transaction Amount (NGN) </td>\n                                            <td style="min-width:70%; width:70%; font-size: 1.5em; border: 1px #000000 solid;">\n                                            '+kendo.toString(kendo.parseFloat(transactionData.amountToCharge),"n2")+'\n                                            </td></tr>\n                                            <tr>\n                        <td style="min-width:30%; width:30%; font-size: 1.5em; font-weight: bold; border: 1px #000000 solid;">\n                                            Transaction Status</td>\n                                            <td style="min-width:70%; width:70%; font-size: 1.5em; border: 1px #000000 solid;">\n                                            Successful\n                                            </td></tr>\n                                            <tr>\n                        <td style="min-width:30%; width:30%; font-size: 1.5em; font-weight: bold; border: 1px #000000 solid;">\n                                            Beneficiary </td>\n                                            <td style="min-width:70%; width:70%; font-size: 1.5em; border: 1px #000000 solid;">\n                                            '+(transactionData.beneficiary.accountNumber+" - "+transactionData.beneficiary.accountName)+'\n                                            </td></tr></table>';}break;// end of "Cash Transfer (Card)" transaction type
case"Cash Transfer (Bank)":// transaction type is "Cash Transfer (Bank)"
...
transactionHistoryDocContent+='Transaction Type </td>\n                                            <td style="min-width:70%; width:70%; font-size: 1.5em; border: 1px #000000 solid;">\n                                            '+transactionData.postcash_transaction_type+'\n                                            </td></tr>\n                                            <tr>\n                        <td style="min-width:30%; width:30%; font-size: 1.5em; font-weight: bold; border: 1px #000000 solid;">\n                                            Transaction Amount (NGN) </td>\n                                            <td style="min-width:70%; width:70%; font-size: 1.5em; border: 1px #000000 solid;">\n                                            '+kendo.toString(kendo.parseFloat(transactionData.amountToCharge),"n2")+'\n                                            </td></tr>\n                                            <tr>\n                        <td style="min-width:30%; width:30%; font-size: 1.5em; font-weight: bold; border: 1px #000000 solid;">\n                                            Transaction Status</td>\n                                            <td style="min-width:70%; width:70%; font-size: 1.5em; border: 1px #000000 solid;">\n                                            Successful\n                                            </td></tr>\n                                            <tr>\n                        <td style="min-width:30%; width:30%; font-size: 1.5em; font-weight: bold; border: 1px #000000 solid;">\n                                            Beneficiary </td>\n                                            <td style="min-width:70%; width:70%; font-size: 1.5em; border: 1px #000000 solid;">\n                                            '+(transactionData.postcash_transferDetails.recipient_account_number+" "+transactionData.postcash_transferDetails.recipient_account_name)+'\n                                            </td></tr></table>';}break;// end of "Cash Transfer (Bank)" transaction type
}// append the generated content to the transaction-history-saver-container
$('#transaction-history-saver-container').append(transactionHistoryDocContent);// convert the generate html content to a png image file/document
return domtoimage.toBlob($('#transaction-history-saver-container').get(0),{cacheBust:true,bgcolor:'#ffffff',width:1880,height:2908});}).then(function(blob){// get the blob content
...
window.resolveLocalFileSystemURL(cordova.file.externalRootDirectory,resolve,reject);});}).then(function(directory){return new Promise(function(resolve,reject){// return the created file which holds the image document
directory.getFile('PostCash-T-'+Date.now()+'.png',{create:true,exclusive:false},resolve,reject);});}).then(function(file){// get the file object
...
$('#app-main-navigator').get(0).topPage.alwaysShowSecurityLockModal=false;// open the saved document/image
return new Promise(function(resolve,reject){cordova.plugins.fileOpener2.open(fileObj.toURL(),"image/png",{error:reject,success:resolve});});}).then(function(){// display a toast message to let user know that file has been saved
window.plugins.toast.showWithOptions({message:"Transaction details saved to file",duration:6000,// 6000 ms
position:"top",styling:{opacity:1,backgroundColor:'#808080',textColor:'#FFFFFF',textSize:14}});}).catch(function(err){// set the flag to inform the app to display Security Lock Modal
...
ons.notification.alert({title:"File Save Failed",messageHTML:'<ons-icon icon="md-close-circle-o" size="30px" '+'style="color: red;"></ons-icon> <span>Sorry, the details of this transaction could not be saved to file. '+'<br>You can try again </span>',cancelable:true});});},/**
         * method is used to trigger the "Send Feedback" operations when the
         * "Send Feedback" button is clicked
         *
         * @param buttonElem
         */sendFeedbackButtonClicked:function sendFeedbackButtonClicked(buttonElem){// inform the user that they are about to share transaction details with the support team
ons.notification.confirm({title:'<ons-icon icon="md-info" size="32px" '+'style="color: blue;"></ons-icon> Privacy Notice',messageHTML:'<span>You are about to send details of your PostCash transaction to the PostCash support team. <br>'+'Do you want to continue?</span>',cancelable:false,buttonLabels:["No","Yes"]}).then(function(buttonIndex){if(buttonIndex===1){// user wants to continue sending feedback
// display message to user
$('#loader-modal-message').html("Preparing Feedback Form...");return $('#loader-modal').get(0).show();// show loader
}else{// user terminated action
throw null;}}).then(function(){// get the specific transaction history which is to be saved
var transactionRef=$(buttonElem).attr("data-transaction-id");return utopiasoftware.saveup.transactionHistoryOperations.getTransactionHistoryById(transactionRef);}).then(function(transactionData){// receive the requested transaction data
if(!transactionData){// no transaction data was found
throw{"message":"Transaction not found."};}return transactionData;}).then(function(transactionData){// receive the transaction data
// set the flag to inform the app NOT to display Security Lock Modal
$('#app-main-navigator').get(0).topPage.alwaysShowSecurityLockModal=false;return new Promise(function(resolve,reject){// display the feedback form and attach the retrieved transaction data
hockeyapp.composeFeedback(resolve,reject,false,transactionData);});}).then(function(){$('#loader-modal').get(0).hide();// hide loader
...
ons.notification.prompt({title:"Security Check",id:"pin-security-check",class:"utopiasoftware-no-style",messageHTML:'<div><ons-icon icon="ion-lock-combination" size="24px" '+'style="color: #b388ff; float: left; width: 26px;"></ons-icon> <span style="float: right; width: calc(100% - 26px);">'+'Please enter your PostCash Secure PIN to proceed</span></div>',cancelable:true,placeholder:"Secure PIN",inputType:"number",defaultValue:"",autofocus:true,submitOnEnter:true}).then(function(userInput){// user has provided a secured PIN , now authenticate it
if(userInput===utopiasoftware.saveup.model.appUserDetails.securePin){// authentication successful
$('#app-main-navigator').get(0).pushPage("change-pin-page.html",{animation:"slide-md"});// navigate
}else{// inform user that security check failed/user authentication failed
ons.notification.alert({title:"Security Check",messageHTML:'<ons-icon icon="md-close-circle-o" size="30px" '+'style="color: red;"></ons-icon> <span>'+'Security check failed. Invalid credentials'+'</span>',cancelable:true});}}).catch(function(){});return;}if(label=="always lock screen"){// 'always lock screen' list item was clicked
...
$('#settings-page #settings-always-lock-screen-switch').trigger("change");return;}}}),_defineProperty(_utopiasoftware$saveu,'updateProfilePageViewModel',{/**
...
if(!ons.isReady()||utopiasoftware.saveup.model.isAppReady===false){setTimeout(loadPageOnAppReady,500);// call this function again after half a second
return;}// listen for the back button event
$('#app-main-navigator').get(0).topPage.onDeviceBackButton=function(){// move to the previous page on the stack
$('#app-main-navigator').get(0).popPage({});};// initialise the update-profile form validation
utopiasoftware.saveup.controller.updateProfilePageViewModel.formValidator=$('#update-profile-form').parsley();// attach listener for the update profile button on the page
$('#update-profile-button').get(0).onclick=function(){// run the validation method for the update profile form
utopiasoftware.saveup.controller.updateProfilePageViewModel.formValidator.whenValidate();};// listen for log in form field validation failure event
utopiasoftware.saveup.controller.updateProfilePageViewModel.formValidator.on('field:error',function(fieldInstance){// get the element that triggered the field validation error and use it to display tooltip
// display tooltip
$(fieldInstance.$element).parent().find('label:eq(0)').addClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).parent().find('label:eq(0)').attr("data-hint",fieldInstance.getErrorsMessages()[0]);});// listen for form field validation success event
...
$('#loader-modal').get(0).hide();}},/**
         ...
         * @param event
         */pageHide:function pageHide(event){try{// remove any tooltip being displayed on all forms on the page
$('#update-profile-page [data-hint]').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$('#update-profile-page [data-hint]').removeAttr("data-hint");// reset the form validator object on the page
utopiasoftware.saveup.controller.updateProfilePageViewModel.formValidator.reset();}catch(err){}},/**
         * method is triggered when the page is destroyed
         * @param event
         */pageDestroy:function pageDestroy(event){try{// remove any tooltip being displayed on all forms on the page
$('#update-profile-page [data-hint]').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$('#update-profile-page [data-hint]').removeAttr("data-hint");// destroy the form validator objects on the page
utopiasoftware.saveup.controller.updateProfilePageViewModel.formValidator.destroy();}catch(err){}},/**
         * method is triggered when the form is successfully validated
         */updateProfileFormValidated:function updateProfileFormValidated(){// display the loader message to indicate that account is being created;
$('#loader-modal-message').html("Updating User Profile...");Promise.resolve($('#loader-modal').get(0).show()).// show loader.
then(function(){// update the app user details object and persist it
utopiasoftware.saveup.model.appUserDetails.firstName=$('#update-profile-form #update-profile-first-name').val();utopiasoftware.saveup.model.appUserDetails.lastName=$('#update-profile-form #update-profile-last-name').val();if($('#update-profile-form #update-profile-email').val()!=""){// check if user has entered an email
utopiasoftware.saveup.model.appUserDetails.email=$('#update-profile-form #update-profile-email').val();}return utopiasoftware.saveup.model.appUserDetails;// return the updated profile details, so it can be persisted
...
ons.notification.alert({title:"Profile Update Failed",messageHTML:'<ons-icon icon="md-close-circle-o" size="30px" '+'style="color: red;"></ons-icon> <span>sorry, your profile could not be updated.<br> You can try again</span>',cancelable:false});});}}),_defineProperty(_utopiasoftware$saveu,'changePinPageViewModel',{/**
         * used to hold the parsley form validation object for the form
         */formValidator:null,/**
         * event is triggered when page is initialised
         */pageInit:function pageInit(event){var $thisPage=$(event.target);// get the current page shown
...
return;}// listen for the back button event
$('#app-main-navigator').get(0).topPage.onDeviceBackButton=function(){// move to the previous page on the stack
$('#app-main-navigator').get(0).popPage({});};// initialise the change-pin form validation
utopiasoftware.saveup.controller.changePinPageViewModel.formValidator=$('#change-pin-form').parsley();// attach listener for the change pin button on the page
$('#change-pin-button').get(0).onclick=function(){// run the validation method for the change pin form
utopiasoftware.saveup.controller.changePinPageViewModel.formValidator.whenValidate();};// listen for log in form field validation failure event
utopiasoftware.saveup.controller.changePinPageViewModel.formValidator.on('field:error',function(fieldInstance){// get the element that triggered the field validation error and use it to display tooltip
// display tooltip
$(fieldInstance.$element).parent().find('label:eq(0)').addClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).parent().find('label:eq(0)').attr("data-hint",fieldInstance.getErrorsMessages()[0]);});// listen for form field validation success event
utopiasoftware.saveup.controller.changePinPageViewModel.formValidator.on('field:success',function(fieldInstance){// remove tooltip from element
$(fieldInstance.$element).parent().find('label:eq(0)').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).parent().find('label:eq(0)').removeAttr("data-hint");});// listen for form validation success
...
$('#loader-modal').get(0).hide();}},/**
         ...
         * @param event
         */pageHide:function pageHide(event){try{// remove any tooltip being displayed on all forms on the page
$('#change-pin-page [data-hint]').removeClass("hint--always hint--info hint--medium hint--rounded hint--no-animate");$('#change-pin-page [data-hint]').removeAttr("data-hint");// reset the form validator object on the page
utopiasoftware.saveup.controller.changePinPageViewModel.formValidator.reset();}catch(err){}},/**
         * method is triggered when the page is destroyed
         * @param event
         */pageDestroy:function pageDestroy(event){try{// remove any tooltip being displayed on all forms on the page
...
cordova.plugins.email.open({app:'mailto',to:['support+7ee364178e62478c8e60933dd5d594d0@feedback.hockeyapp.net']},resolve);}).then(function(){// after email app has been opened
