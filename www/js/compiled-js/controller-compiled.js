...
$('#my-cards-add-card-button',$thisPage).attr("disabled",true);});}},/**
         ...
         */loadCardData:function loadCardData(){// return the Promise object
return new Promise(function(resolve,reject){// read the user's cards data from secure storage
Promise.resolve(intel.security.secureStorage.read({'id':'postcash-user-cards'})).then(function(instanceId){// read the content of the securely stored cards data
return Promise.resolve(intel.security.secureData.getData(instanceId));},function(errObject){if(errObject.code==1){// the secure card storage has not been created before
...
resolve(secureCardDataArray);}).catch(function(err){// reject the Promise
reject(err);});});},/**
...
$('#add-card-card-number',$thisPage).characterCounter();// remove the progress indeterminate loader
