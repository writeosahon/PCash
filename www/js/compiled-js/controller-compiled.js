...
$('#add-card-page .page__content').scrollTop(0);$('#secure-storage-modal').get(0).hide();// hide loader
// inform user that add has been successfully added to secure storage
Materialize.toast('New card added successfully',4000);},4000);}).catch(function(err){ons.notification.alert({title:"Save Error",messageHTML:'<ons-icon icon="md-close-circle-o" size="30px" '+'style="color: red;"></ons-icon> <span>'+(err.message||"")+' Sorry, this card could not be added. '+'<br>You can try again'+'</span>',cancelable:true});});}},/**
...
$('#add-account-page #add-account-form').get(0).reset();// reset the account number validation check and all its related actions
...
$('#add-recipient-page #add-recipient-form').get(0).reset();// reset the account number validation check and all its related actions
...
$('#add-recipient-page .page__content').scrollTop(0);$('#secure-storage-modal').get(0).hide();// hide loader
// inform user that user's bank account has been successfully added to secure storage
Materialize.toast("New recipient's bank account added",4000);},4000);}).catch(function(err){ons.notification.alert({title:"Save Error",messageHTML:'<ons-icon icon="md-close-circle-o" size="30px" '+'style="color: red;"></ons-icon> <span>'+(err.message||"")+" Sorry, this recipient's bank account could not be added. "+'<br>You can try again'+'</span>',cancelable:true});});}},/**
         * method is used to check if user has asked for bank account number to be validated.
         * It sets or remove the appropriate attributes need to activate or deactivate
         * remote account validation
         */isbankAccountNumberValidated:function isbankAccountNumberValidated(checkElem){// check if user wants acct number validated or not
